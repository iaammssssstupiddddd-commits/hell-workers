...
description: Run hell-workers native acceptance and renderer, GPU, or performance validation through the established no-prompt terminal launcher, with sequential Capture and Memory builds, fail-closed artifact monitoring, and bounded resource use. Use whenever the user asks for 実機確認, 実機テスト, native or manual acceptance, X11 or Wayland renderer evidence, or asks the agent to test without permission prompts; also use after changes whose completion criteria require an actual window, adapter, or native allocator evidence.
---
...
Run only the returned direct `kitty` command. The v5 validator requires all 18
...
clean worktree; v5 hashes that full local asset view at plan, rechecks it before
