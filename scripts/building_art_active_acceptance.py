"""Native active-building reference: six Capture runs, then three Memory runs.

Use through dev.py validation plan. Requires ordinary repeated production and hauling.
This is not art approval or a frozen performance budget.
"""
...
PROFILE = "building-art-active-nine-reference-v1"
MATRIX = (("capture", "small", 29), ("capture", "medium", 116), ("memory", "medium", 116))
SEED = 20260920


def command(root: Path, instrumentation: str, size: str, souls: int, adapter: str) -> list[str]:
    return ["python3", "scripts/perf.py", "run", "--workload", "building-art-active",
            "--sizes", size, "--renders", "gpu", "--seed", str(SEED), "--repeat", "3",
            "--preflight-runs", "0", "--souls", str(souls), "--familiars", str(2 if size == "small" else 8),
...
def require_identity(repo: Path, expected: dict) -> None:
    native.require(not native.git_dirty_paths(repo), "active reference requires a fully clean committed subject")
    native.require(identity(repo) == expected, "active reference source/driver/assets changed")
...
def check_root(repo: Path, root: Path) -> None:
    ...
    native.require_persistent_storage(root, label="building active reference")
...
def plan(args) -> int:
    ...
    root = Path(args.job_root).resolve() if args.job_root else native.unique_job_root(repo, "building-art-active")
    ...
    native.print_json({"status": "blocked" if failures else "ready", "profile": PROFILE, "job_root": str(root),
                       **frozen, "resources": resources, "failures": failures, "launcher_command": launcher,
                       ...
                       "execution_contract": {"capture_runs": 6, "memory_runs": 3, "warmup_secs": 30,
                                              "measure_secs": 60, "parallel_processes": 1, "active_simulation": True}})
...
def verify_session(root: Path, instrumentation: str, size: str, souls: int, frozen: dict, adapter: str) -> dict:
    ...
    case = Case("building-art-active", size, "gpu", SEED, souls, 2 if size == "small" else 8)
    expected_matrix = {"workload": case.workload, "sizes": [size], "renders": ["gpu"], "seed": SEED,
                       ...
                       "souls": souls, "familiars": case.familiars, "familiar_policies": ["baseline"],
    ...
    for number in range(1, 4):
        ...
        fixture = native.read_json(run / "data/building_art_active.json")
        native.require(fixture["frames"] == len(samples), "fixture observation/frame sample coverage differs")
        ...
        observations.append({"run": number, "metrics": metrics, "adapter": validation.adapter,
                             "layout_sha256": fixture["layout_sha256"], "load": {key: fixture[key] for key in ("lanes", "particles_max", "particles_sum", "ui_particles_max", "ui_particles_sum", "frames", "seconds")}})
...
def verify(root: Path) -> dict:
    ...
    return {"status": "pass", "profile": PROFILE, "subject_commit": manifest["identity"]["subject_commit"],
            "capture_runs": 6, "memory_runs": 3, "active_simulation": "ordinary-production-and-hauling", "sessions": sessions}
...
if __name__ == "__main__":
    try:
        ...
        print(f"building-art-active: {error}", file=sys.stderr)
