...
error_log_file="$log_dir/build_error_${timestamp}.log"
combined_log_file="$log_dir/build_combined_${timestamp}.log"

echo "Running cargo check..."
echo "Error log: $error_log_file"
echo "Target dir: $target_dir"

if cargo check --manifest-path "${PROJECT_ROOT}/Cargo.toml" --target-dir "$target_dir" >"$combined_log_file" 2>"$error_log_file"; then
    build_exit_code=0
else
    build_exit_code=$?
fi
