...
Linear標準受付へ移行する[現行計画](docs/plans/orca-parallel-development-plan-2026-09-20.md)では、既存の権限・資源制御と固定reviewerを再利用します。専用試験issue `TAK-5` の作成・更新・再読取り・worktree関連付けと、固定snapshotからの統括相談・同一session追記、Codex固定reviewerのread-only Task一巡を受入済みです。Cursor B連携・編集運用・自動同期は受入前です。
