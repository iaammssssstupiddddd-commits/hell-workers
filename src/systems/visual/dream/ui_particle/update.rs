...
use super::super::components::{DreamGainUiParticle, DreamIconAbsorb, DreamTrailGhost};
...
fn update_standard_particle(
    ...
    node.width = Val::Px(DREAM_UI_PARTICLE_SIZE * effective_mass.sqrt() * cluster_scale * shrink * width_scale);
    node.height = Val::Px(
        DREAM_UI_PARTICLE_SIZE * effective_mass.sqrt() * cluster_scale * shrink * length_scale,
    );
    ...
    if particle.trail_cooldown <= 0.0 && dist_ratio > 0.15 {
        ...
        let trail_size = DREAM_UI_PARTICLE_SIZE * effective_mass.sqrt() * cluster_scale * shrink * DREAM_UI_TRAIL_SIZE_RATIO;
...
fn spawn_trail_ghost(
    ...
    if speed > 1.0 {
        let angle =
            vel_dir.y.atan2(vel_dir.x) - std::f32::consts::FRAC_PI_2;
