...
fn handle_pouring(
    ...
    let time_delta = 1.0; // 固定値として扱う（timeパラメータが渡されないため）
    let new_progress = progress + time_delta * 1.0; // 1秒で注ぐ
    if new_progress >= 1.0 {
        // 注ぎ完了！バケツを空に戻す
        commands
