...
impl EntityInspectionQuery<'_, '_> {
    pub fn build_model(&self, entity: Entity) -> Option<EntityInspectionModel> {
        ...
        } else if let Ok(bp) = self.q_blueprints.get(entity) {
        ...
        } else if let Ok((familiar, op)) = self.q_familiars.get(entity) {
        ...
        } else if let Ok(item) = self.q_items.get(entity) {
        ...
        if let Ok((des, issued_by_opt, task_workers_opt)) = self.q_designations.get(entity) {
            ...
            if let Some(issued_by) = issued_by_opt {
                if let Ok((fam, _)) = self.q_familiars.get(issued_by.0) {
