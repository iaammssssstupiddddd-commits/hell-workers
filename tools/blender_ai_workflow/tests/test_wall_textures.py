...
class WallTextureTests(unittest.TestCase):
    ...
    def test_valid_texture_set_passes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(report["textures"]["normal"]["path"], "wall_normal.png")

    def test_rejected_normal_is_not_read_or_reported(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.fixture(root)
            (root / "wall_normal.png").unlink()
            report = self.validator.validate_textures(
                root,
                normal_decision="rejected",
                normal_sampling=None,
                normal_convention=None,
            )
            self.assertEqual(report["normal_decision"], "rejected")
            self.assertNotIn("normal", report)
            self.assertEqual(set(report["textures"]), {"albedo", "emissive"})

    def test_rejected_normal_cannot_retain_sampling_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.fixture(root)
            with self.assertRaisesRegex(
                self.validator.TextureError, "must not retain sampling metadata"
            ):
