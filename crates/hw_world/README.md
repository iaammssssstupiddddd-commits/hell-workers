...
| `mapgen.rs` | `generate_base_terrain_tiles()` と `generate_world_layout()`（WFC + validate + resource 配置 + retry/fallback + river/sand/rock-field 派生マスク） |
...
| `AnchorLayout`, `WorldMasks`, `mapgen::wfc_adapter`, `mapgen::resources`, `rock_fields` | `GeneratedWorldLayout` を root Resource に包んで startup / map spawn / regrowth 初期化へ接続する app shell |
