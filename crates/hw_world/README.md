...
ゲームワールドの地形生成・管理、座標変換ユーティリティ、A* 経路探索と Boolean 到達判定用の連結成分 cache を提供するクレート。
...
| `pathfinding/` | A* 経路探索と `WalkabilityConnectivityCache`（下記詳細参照） |
...
| `door_systems.rs` | `DoorVisualHandles`、`apply_door_state`、1候補に対するpure auto-open/keep-open rule |
| `terrain_visual.rs` | source-aware 障害物同期（`ObstaclePositionIndex` / `obstacle_sync_system`）と `TerrainChangedEvent`（`Message`）発行 |
| `spatial.rs` | downstream world/AI helperが共有する`SpatialGridOps` trait契約 |
...
### 公開 runtime facade と crate 内 core

```rust
// hw_world 外の runtime caller が使う公開 API
find_path_with_budget(...)                         // direct core A* 1回を予算付きで実行
find_path_to_adjacent_with_budget(...)             // 隣接goalのcore A*を予算付きで実行
find_path_to_boundary_with_budget(...)             // footprint境界への multi-goal A*
find_path_world_waypoints_with_budget(...)         // direct / adjacent を別枠で課金

// hw_world 内だけの pub(crate) core API（mapgen validation / unit test）
find_path(...)
...
`WalkabilityConnectivityCache` は runtime の Boolean 到達判定専用 Resource である。
`WorldMap.obstacle_version` が一致する間は、同じ dense component ID 配列を O(1) で参照する。
walkable target は start と同じ連結成分かを、blocked target は A* と同じ斜め corner-cutting 規則で有効な隣接 goal が同じ成分かを判定する。実際の waypoint 列、Door cost、mapgen validation は引き続き A* を使う。

Open/Closed Door は walkability topology を変えないため cache を再構築しない。Locked 切替、地形・橋・障害物などで `obstacle_version` が変わった場合だけ、次の問い合わせで一度 flood-fill する。cache は save payload に含めず、world replacement 時に reset する。
...
| Room 検出 core + ECS adapter + `Room` / border-line用`RoomOverlayTile` / 管理Resource | `plugins/logic.rs` / `plugins/visual.rs`からの直接登録 |
...
| `obstacle_sync_system`、door state/pure candidate rule | rootのplugin wiring。doorのSoul index adapterは`hw_spatial::door_proximity`、専用handle Resourceはstartupから注入 |
...
`WorldMapRead` / `WorldMapWrite` はシステム引数として使用する。runtime で waypoint が必要な場合は、`PathfindingContext` と共有budgetを併用する:
```rust
fn my_system(
    world_map: WorldMapRead,
    mut context: Local<PathfindingContext>,
    mut budget: ResMut<RuntimePathSearchBudget>,
) {
    let outcome = find_path_with_budget(
        world_map.as_ref(),
        &mut context,
        &mut budget,
