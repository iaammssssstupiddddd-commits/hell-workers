...
| `obstacle_sync_system` / `door_auto_open_system` / `door_auto_close_system` | root の plugin wiring。obstacle sync は `ObstacleSyncSet` で Actor/pathfinding より前に実行し、door system は専用 handle Resource を startup から受け取る |
