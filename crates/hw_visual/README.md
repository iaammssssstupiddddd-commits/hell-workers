...
| `material/topdown_structural_material.rs` | 建物用 `TopDownStructuralMaterial`。建築進捗、directional shadow style、室内 Light Field を保持し、シェーダは `assets/shaders/topdown_structural_material*.wgsl` |
| `material/terrain_surface_material.rs` | `TerrainSurfaceMaterial`（地形専用共有 material）、`TerrainIdMap` 前提の LOD / prepass / directional shadow style / 室内 Light Field 対応。シェーダは `assets/shaders/terrain_surface_material*.wgsl` |
