...
`events.rs` には root adapter 連携用の bridge event も含む。たとえば `DriftingEscapeStarted` と `SoulEscaped` は `soul_ai` の drifting 処理から root `PopulationManager` 更新を切り離すための cross-crate event で、実際のカウンタ更新や cooldown 開始は root observer が担当する。
