...
use hw_jobs::construction::{FloorTileBlueprint, FloorTileState};
...
pub fn floor_site_tile_demand<'a>(
    ...
    floor_tiles
        ...
        .map(|tile| match resource_type {
            ResourceType::Bone if tile.state == FloorTileState::WaitingBones => {
                FLOOR_BONES_PER_TILE.saturating_sub(tile.bones_delivered) as usize
            }
            ResourceType::StasisMud if tile.state == FloorTileState::WaitingMud => {
