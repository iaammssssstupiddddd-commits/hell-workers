...
use hw_jobs::construction::{WallTileBlueprint, WallTileState};
...
pub fn wall_site_tile_demand<'a>(
    ...
    wall_tiles
        ...
        .map(|tile| match resource_type {
            ResourceType::Wood if tile.state == WallTileState::WaitingWood => {
                WALL_WOOD_PER_TILE.saturating_sub(tile.wood_delivered) as usize
            }
            ResourceType::StasisMud if tile.state == WallTileState::WaitingMud => {
