...
- `blueprint_auto_gather_system` は対応する `TransportRequest` を需要の正本とする。request が無い、または `issued_by` が Active Familiar / Yard の `owner_infos` に登録できない場合は自動 `Chop` / `Mine` を発行しない
- AutoGather の供給控除には owner から到達可能かつ実際に task finder が発見できる資材・既存指定だけを使う。Bridge の Wood/Rock 代替需要は到達可能候補を比較する
