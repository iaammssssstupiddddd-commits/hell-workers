...
| `task_area_visual.rs` | `TaskAreaMaterial` のre-exportとroot `TaskContext`依存の境界ビジュアル更新 |
| `building3d_cleanup.rs` | 3D 建物エンティティのクリーンアップ |
| `camera_sync.rs` | カメラ同期 |

## TaskAreaMaterial

`hw_visual/src/task_area_visual.rs`で定義され、`HwVisualPlugin`で登録されるカスタムマテリアル。
...
| root `TaskContext` 依存の update | `TaskAreaMaterial`の定義・登録、blueprint / haul / dream / gather / soul などの表示同期 |
