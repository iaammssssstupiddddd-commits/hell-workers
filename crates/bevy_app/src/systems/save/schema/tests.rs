...
fn stockpile_policy_round_trips_with_yard_owner_entity_remap() {
    ...
    let mapped_legacy_only_stockpile = entity_map[&legacy_only_stockpile];
    assert_eq!(
    ...
    assert_eq!(
        destination.get::<StockpilePolicy>(mapped_legacy_only_stockpile),
        Some(&StockpilePolicy {
            acceptance: StockpileAcceptance::Only(ResourceType::Wood),
            inbound_priority: TransportPriority::Normal,
            target_amount: 5,
...
fn pre_checklist_stockpile_acceptance_ron_body_deserializes() {
    ...
    // Fixed from the pre-checklist v1 representation. This must not be generated by the
    // current serializer, otherwise a variant/path rename could update both sides unnoticed.
    const PRE_CHECKLIST_BODY: &str = r#"(
  resources: {},
  entities: {
    1: (
      components: {
        "hw_logistics::zone::StockpilePolicy": (
          acceptance: Any,
          inbound_priority: Normal,
          target_amount: 10,
          allow_export: true,
        ),
      },
    ...
    2: (
      components: {
        "hw_logistics::zone::StockpilePolicy": (
          acceptance: Only(Wood),
          inbound_priority: High,
          target_amount: 6,
          allow_export: false,
        ),
      },
    ...
    let type_registry = app.world().resource::<AppTypeRegistry>().clone();
    let registry = type_registry.read();
    let mut ron_deserializer = ron::de::Deserializer::from_str(PRE_CHECKLIST_BODY).unwrap();
    let legacy_world = WorldDeserializer {
        type_registry: &registry,
        load_from_path: &mut NoAssetLoad,
    }
    .deserialize(&mut ron_deserializer)
    ...
    let mut policies = destination
        .query::<&StockpilePolicy>()
        .iter(&destination)
        .copied()
        .collect::<Vec<_>>();
    policies.sort_by_key(|policy| policy.target_amount);
    assert_eq!(
        policies,
        vec![
            StockpilePolicy {
