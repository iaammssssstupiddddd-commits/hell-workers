...
fn rehydrated_rock_item_uses_the_small_ground_item_image() {
    ...
    let mut soul_handles = empty_soul_task_handles();
    soul_handles.icon_rock_small = game_assets.icon_rock_small.clone();

    let sprite = item_sprite(ResourceType::Rock, &game_assets, &soul_handles);

    assert_eq!(sprite.image, game_assets.icon_rock_small);
    assert_ne!(sprite.image, game_assets.rock);
