            ...
            [
                ...
                HelpEntry::new(
                    ...
                    [
                        ...
                        "FPSは左上の時間表示の隣に表示します。開発用パネルはデバッグ表示が有効なときだけ開き、通常の地図補助表示はデバッグ設定に左右されません。",
                ...
                HelpEntry::new(
                    ...
                    "通知",
                    [
                        "短い結果は左上に最大1件表示します。重要な履歴は右下の「通知」から確認でき、最後に届いてからの経過時間も表示します。再保存などの操作ボタンは履歴だけに表示します。通知を読んでも「要対応」の問題が解決した扱いにはなりません。",
