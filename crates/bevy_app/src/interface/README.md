...
root-owned resultから安全なUI通知へのadapterもここに置く。

task dashboard は `ui/panels/task_list/view_model.rs` が producer diagnostics を表示状態へ縮約し、
`actions.rs` が positive allow-list と live revalidation を通して owner別priority/cancelへ接続する。
Pause/Modal中もaction intentをdrainし、`TaskActionOutcome`だけを`ToastOnly`通知へ変換する。
詳細は `ui/README.md` と [docs/task_list_ui.md](../../../../docs/task_list_ui.md) を参照。
