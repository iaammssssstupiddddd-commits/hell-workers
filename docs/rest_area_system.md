...
Bevy 0.19 の **ECS Relationships** を活用し、休憩所の定員管理や予約状態を型安全に管理しています。
...
### 3. 休憩 (Resting)
休憩所の歩行可能な入口または隣接マスに到着すると、ワーカーはその位置のまま `RestingIn` 状態になります。
- **非表示化**: ワーカーのスプライトは非表示（`Visibility::Hidden`）になります。
- **座標保持**: 入所処理は`Transform`を変更せず、pathだけをclearします。
- **回復**: 滞在中、疲労とストレスが継続的に減少します。
- **固定時間**: 3分間（`REST_AREA_RESTING_DURATION` 秒）、滞在が維持されます。

### 4. 退出とクールダウン (Exit & Cooldown)
所定の時間が経過すると休憩所から退出します。
- **再表示位置**: 入所時から保持していた歩行可能な入口/隣接位置で`Visibility::Visible`へ戻ります。退出処理も`Transform`を変更しません。
...
- 休憩所のambient Dream world粒子は滞在人数に応じてスケールしますが、Dream獲得量やUI particle massには数えません
...
## 7. 関連ファイル
- `crates/hw_soul_ai/src/soul_ai/decide/idle_behavior/`: 休憩の意思決定、予約、到着判定。
- `crates/hw_soul_ai/src/soul_ai/execute/idle_behavior_apply.rs`: 休憩所への入退所処理、非表示化、クールダウン中の予約/入所拒否。
- `crates/hw_soul_ai/src/soul_ai/update/rest_area_update.rs`: 休憩効果の適用、per-soul dream 放出、退出判定。
- `crates/hw_core/src/constants/dream.rs`: `DREAM_DRAIN_RATE`, `DREAM_DRAIN_RATE_REST` など Dream 放出定数。
- `crates/hw_visual/src/dream/particle.rs`: 休憩所からのDreamパーティクルのスケーリングと生成。
- `crates/hw_core/src/relationships.rs`（`hw_core::relationships::*` として re-export）: `RestingIn`, `RestAreaOccupants` 等の Relationship 定義。
- `crates/hw_jobs/src/model.rs`: `RestArea` コンポーネントの定義本体。
- `crates/bevy_app/src/systems/jobs/mod.rs`: `RestArea` の公開 re-export 層。
