...
## 7. 関連ファイル
- `crates/hw_ai/src/soul_ai/decide/idle_behavior/`: 休憩の意思決定、予約、到着判定。
- `crates/hw_ai/src/soul_ai/execute/idle_behavior_apply.rs`: 休憩所への入退所処理、非表示化、クールダウン中の予約/入所拒否。
- `crates/hw_ai/src/soul_ai/update/rest_area_update.rs`: 休憩効果の適用、per-soul dream 放出、退出判定。
