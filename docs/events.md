...
| `BuildingCompletedEvent` / `BuildingCompletedVisualMessage` | `Event` / `Message` (`hw_jobs::events`) | `building_completion_system` が完成建物を生成した後、`publish_building_completed` から各1回発行 | `hw_soul_ai`のdomain Observer / root `work_guide::record_building_completion` (`GameSystemSet::Visual`) | EventはWorldMapとObstaclePositionを確定。Messageは元の `blueprint_entity` だけを渡し、ガイドが追跡した同一予定の完成を記録する。取消・別予定の完成では達成しない。元予定はdespawn済みでよく、生存を要求しない。root `MessagesPlugin` inventoryが登録とworld replacement時clearを所有する |
