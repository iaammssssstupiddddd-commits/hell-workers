...
| `UiIntent::SetSoulSpaActiveSlots` | `Message` variant (`hw_ui::UiIntent`) | Soul Spa情報パネルの−/+ | root `handle_ui_intent` | exact target、生存、Operational phaseを再検証し、0〜4へclampして`SoulSpaSlotsChangeOutcome`を同じUpdateに1件発行する |
...
| `UiIntent::SetPowerConsumerPriority` | `Message` variant (`hw_ui::UiIntent`) | Power Consumer情報パネルのPriority | root `handle_ui_intent` | exact target、生存、`PowerConsumer + PowerConsumerPolicy`を再検証し、typed outcomeを同じUpdateに1件発行する |
