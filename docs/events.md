...
| `TerrainChangedEvent` | `Message`（`hw_world::terrain_visual`） | `obstacle_cleanup_system`（`GameSystemSet::Logic`） | `terrain_material_sync_system`（`MessageReader`、`GameSystemSet::Visual`） | 障害物撤去後に `WorldMap` 上の該当タイルが Dirt へ変わったとき `idx` を通知し、`MeshMaterial3d<SectionMaterial>` を `Terrain3dHandles` に合わせて差し替え。登録は `VisualPlugin::add_message::<TerrainChangedEvent>()` |
