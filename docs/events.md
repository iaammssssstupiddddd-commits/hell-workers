...
| `BuildingCompletedEvent` / `BuildingCompletedVisualMessage` | `Event` / `Message` (`hw_jobs::events`) | rootの`commit_completed_building`がWorldMap占有・完成Entity・companion昇格・Blueprint除去を確定した後、`publish_building_completed`から各1回発行 | `hw_soul_ai`のdomain Observer / root `work_guide::record_building_completion` (`GameSystemSet::Visual`) | EventのObserverは確定済みWorldMapを読み、通行を塞ぐ建物の`ObstaclePosition`生成と必要なSoul退避を行う。WorldMap占有の移譲は発行元transactionが所有する。Messageは元の `blueprint_entity` だけを渡し、ガイドが追跡した同一予定の完成を記録する。取消・別予定の完成では達成しない。元予定はdespawn済みでよく、生存を要求しない。root `MessagesPlugin` inventoryが登録とworld replacement時clearを所有する |
...
| `SoulSpaSlotsChangeOutcome` | `hw_energy::soul_spa` / root `MessagesPlugin` | root `handle_ui_intent` のlive target/phase検証 | Soul Spa slot通知adapter（同じ`Update::NotificationSystemSet::Adapt`） | slot intentごとに1件。exact/clamp/stale/unsupported/phase unavailableをtyped resultで表し、失敗時はsiteを変更しない |
...
| `UserFacingNotification` | `hw_ui::notifications` / `HwUiPlugin` | save/load root adapter、Zone配置のrelease結果など | `NotificationSystemSet::Reduce` → `Present`（同じUpdate） | 表示専用Message。stable key、severity、safe title/body、retentionを持つ。2秒dedupe、toast 1件、重要履歴64件へreduce。Zoneは成功/拒否をToastOnlyで通知 |
...
| `UiIntent::AdjustTaskPriority` / `CancelTask` | `Message` variant (`hw_ui::UiIntent`) | task dashboard action button system | root `UiDomainCommitSet` の `apply_task_action_intents_system` | Entity、expected WorkType、adjustment/cancel kindを渡す。Pause/Modal中もreaderをdrainし、live positive capabilityを再検証して`TaskActionOutcome`を1件発行する。generic UI/save handlerより先に同frame commitする |
| `StockpilePolicyChangeRequest` | `Message` (`hw_logistics::stockpile_policy_change`、root inventory登録) | root `UiDomainCommitSet`（単一・矩形の `UiIntent::ApplyStockpilePolicy` を変換） | `apply_stockpile_policy_change_requests_system`（Interface、通知Adapt前） | targetsを決定的に重複除去し、生存と`Stockpile + StockpilePolicy`を再検証してpatchを適用。同値componentは書き戻さず、在庫relationshipを変更しない |
...
| `UiIntent::SetSoulSpaActiveSlots` | `Message` variant (`hw_ui::UiIntent`) | Soul Spa情報パネルの−/+ | root `UiDomainCommitSet` | exact target、生存、Operational phaseを再検証し、0〜4へclampして`SoulSpaSlotsChangeOutcome`を同じUpdateに1件発行する |
| `UiIntent::CancelSoulSpaConstruction` / `SoulSpaConstructionCancelRequest` | `UiIntent` variant (`hw_ui`) / `Message` (`hw_energy::soul_spa`、root inventory登録) | Constructing Soul Spa情報パネル、またはTask Dashboardのwell-formed `DeliverToSoulSpa + TargetSoulSpaSite` cancel | pause中はroot adapterがrequestを残さず`Paused` outcomeへ終端。通常時は`soul_spa_construction_cancellation_system`（`TaskOwnerCancellationSet::Cancel`）がexact 2×2 owner、tile、関連worker/request、power shapeを事前検証し、site単位cleanupと実搬入Bone返却を一括適用してrequestごとにoutcomeを1件返す |
| `UiIntent::SetPowerConsumerPriority` | `Message` variant (`hw_ui::UiIntent`) | Power Consumer情報パネルのPriority | root `UiDomainCommitSet` | exact target、生存、`PowerConsumer + PowerConsumerPolicy`を再検証し、typed outcomeを同じUpdateに1件発行する |
| `DeconstructionDesignationRequest` / `DeconstructionDesignationOutcome` | `Message` (`hw_jobs::deconstruction`、root inventory登録) | OrdersのDeconstruct pointer ownerが1 press/releaseにつき1件発行 | `deconstruction_designation_system`（Familiar / Transport Perceive前） | world epochとhit entityを再検証し、canonical completed rootへ専用orderを最大1件作る。targetの既存Designationを変更せず、success/reject receiptをrequestごとに返す。world replacementではbufferをclearする |
| `DeconstructionCommitRequest` / `DeconstructionCommitOutcome` | `Message` (`hw_jobs::deconstruction`、root inventory登録) | Soul `Deconstruct` executorがdismantling完了時に1 identityにつき1件発行 / root finalizerが各requestのreceiptを発行 | `deconstruction_finalizer_system`（Soul ExecuteとBuilding completionのdeferred反映後、ObstacleSync前） | world epoch、worker exact identity、order、targetを再検証する。cancelを先に処理しtarget単位で直列化、winnerだけを正常完了させ、duplicate/stale/owner/recovery/Move failureをtyped resultへする。world replacementではrequest/outcomeをclearする |
| `DeconstructionCancelRequest` / `DeconstructionCancelOutcome` | `Message` (`hw_jobs::deconstruction`、root inventory登録) | Task Dashboardの確認付きcancel adapter | `deconstruction_finalizer_system`（commit drainより先） | UIはcomponentを変更せずrequestだけを発行する。claim取得前だけorder workerをall-or-noneでabortしてorder/pendingを閉じる。claim中、stale world/orderはworld非変更のtyped result。world replacementではrequest/outcomeをclearする |
