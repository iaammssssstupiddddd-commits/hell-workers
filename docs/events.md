...
| `OnTaskCompleted` | `EntityEvent+Message` | `task_execution_system`（`TaskEndDisposition::Completed` のみ） | やる気 Observer + speech システム（`MessageReader<OnTaskCompleted>`） | やる気ボーナス付与（Chop/Mine:+2%, Haul:+1%, Build系:+5%）/ 完了時の発話 |
...
| `TerrainChangedEvent` | `Message`（`hw_world::terrain_visual`） | `obstacle_cleanup_system`（`GameSystemSet::Logic`） | `terrain_id_map_sync_system`（`MessageReader`、`GameSystemSet::Visual`） | 障害物撤去後に `WorldMap` 上の該当タイルが Dirt へ変わったとき `idx` を通知し、`TerrainIdMap` の対応ピクセルを書き換えて共有 `TerrainSurfaceMaterial` の見た目を更新する。**chunk entity（`TerrainChunk`）の再生成は不要**。shader が world-space で texture を参照するため、texture 1 ピクセル書き換えだけで全 chunk の見た目が更新される。登録は `VisualPlugin::add_message::<TerrainChangedEvent>()` |
