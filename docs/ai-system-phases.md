...
**システム例**:
- Soul AI: `idle_behavior_decision_system`, `mud_mixer_auto_refine_system`, `blueprint_auto_build_system`, `escaping_decision_system`（0.5秒間隔, 初回即時）
...
**システム例**:
- Soul AI: `apply_designation_requests_system`, `apply_task_assignment_requests_system`, `task_execution_system`, `idle_behavior_apply_system`, `escaping_apply_system`, `hw_logistics::apply_reservation_requests_system`, `gathering_spawn_system`
...
| `DesignationRequest` | Designation 発行（`DesignationOp::Issue`） | `soul_ai/decide/work/auto_refine.rs`, `transport_request/producer/*` | `execute/designation_apply.rs` |
