...
- **三叉路**: 全境界エッジを合わせたグラフで次数 ≥ 3 のグリッドコーナーでは、法線ノイズ変位を 0 にして多地形交点で帯がずれないようにする。
