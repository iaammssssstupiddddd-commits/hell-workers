...
**terrain_surface_material.wgsl との連携**: 異カテゴリ境界（草↔土など）の `should_blend_pair` ブレンドはリボンが担当するため、シェーダ側のクロスカテゴリブレンドは除去済み。同一カテゴリ内のゾーントーン差（草ゾーン↔中立↔土ゾーン）のみシェーダ側でソフトブレンドを残す。
