...
- 解決したい課題: 停滞タスクの理由を安全に可視化し、絞り込み・優先度変更・owner別キャンセルを提供する
...
- 変更内容:
  ...
  - 同一 fixture の candidate / source scan / connectivity / runtime A* counter baseline の計測契約を定義する。
- 変更ファイル:
...
- 完了条件:
  - [x] 現行の全 early return がレビュー済み台帳の分類を持つ。M3 完了後は typed result の
    exhaustive match と WorkType fixture により、新しい未分類分岐を compile / test で検知できる。
  - [x] coverage partial を blocker にしない pure test がある。
  - [x] 離れた 2 つの TaskArea の非 global task では他方の Familiar を `NotApplicable` として分母から外し、
    同じ配置の global Build / Yard-owned task は `Applicable` のまま、universe 走査未完了は unknown / Pending とする
    pure test がある。
  - [x] 同じ Familiar の Soul 数 / source 候補数、worker の spawn / sort 順を変えても、local reducer の
    terminal outcome と producer merge 後の代表理由が変わらない。
  - [x] applicable producer が 1 つでも未評価なら blocker を確定しない。
  - [x] 現行 bool の「true だが request 未発行」経路を再現する test がある。
  - [x] submitted + TaskWorkers 0、submitted + same-frame relationship removal、TaskWorkers > 0 をそれぞれ
    Pending / Pending / Working とする contract test がある。
  - [x] wheelbarrow arbitration の no vehicle / 全台 PushedBy / no source / capacity / reservation / batch wait /
    grant competition が header / typed outcome へ分類され、lease 不在や available query だけから理由を推測する
    分岐が台帳上 0 件である。
  - [x] capability matrix が provenance 不明、Blueprint priority、Move、auto request / gather を read-only とする。
  - [x] hidden dashboard baseline の schema / fixture / counter 要件が定義され、実装・計測は性能フォローアップへ移管されている。
- 検証:
...
- 完了条件:
  - [x] counters は producer × applicable evaluator 1 vote の固定長・saturating で、未観測 task を seed せず stale record を保持しない。
  - [x] 1 Familiar 内の複数 worker / source outcome は出現有無 + 固定順位で 1 票になり、件数・spawn 順で変わらない。
  - [x] submitted が 1 件あればその producer cycle を Blocked にせず、`TaskWorkers` がなければ Pending、
    request 未提出かつ全 producer terminal rejection + complete coverage だけが Blocked になる。
  - [x] 離れた 2 つの TaskArea の非 global task が互いの Familiar を待たずに確定し、global Build / Yard-owned task は
    両方の evaluator を分母に含め、applicable evaluator の未評価だけが coverage partial / Pending になる。
  - [x] root の既存 Designation universe と cycle header から zero roster を表示でき、record absent、new task、
    spatial index 未反映、dirty input、short-circuit task は Pending になる。
  - [x] `SharedResourceCache::begin_frame()` だけで availability generation が進まない。
  - [x] auto-gather Commands を `AutoGatherFlush` が反映した後に final revision sync、delegation の順で観測し、
    root system と anonymous `ApplyDeferred` の偶然の実行順へ依存しない。
  - [x] submitted 後に assignment が同 frame で解除された fixture でも、`TaskWorkers` 0 なら Pending となる。
  - [x] task deletion / Designation removal / producer cycle replacement / world replacement で diagnostics に
    stale Entity が残らない。
  - [x] runtime A* を呼ばず、既存 connectivity call 数を変えない。
- 検証:
...
- 変更内容:
  ...
  - hidden / visible / filter変更で AI counter が同一である統合テスト要件を定義し、専用 perf fixture は性能フォローアップへ移管する。
- 変更ファイル:
...
- 完了条件:
  - [x] Working / 5 blocker / Pending の formatter が空文字にならない。
  - [x] submitted のみを accepted / Working と誤認せず、current `TaskWorkers` がなければ Pending となる。
  - [x] filter 4 種、sort 4 key、direction、Entity tie-break を pure test で網羅する。
  - [x] priority tier が row color、summary、filter、sort で一致する。
  - [x] cycle / revision 数値だけの更新で子 UI を rebuild しない。
  - [x] action 未実装の M2 でも row focus / pin と Modal / Pause capture が退行しない。
  - [x] Build の別 producer 未評価や深い policy 未評価を Blocked にせず Pending とする。
  - [x] reset 境界内で header / empty row / wrapper を除去し、旧 Entity を次 Update まで残さない。
  - [x] dashboard hidden / visible / filter変更の counter 完全一致検証が性能フォローアップへ移管されている。
- 検証:
...
- 変更ファイル:
  ...
  - `crates/bevy_app/src/interface/ui/panels/task_list/view_model.rs`
- 完了条件:
  - [x] 対象 WorkType の attempt が submitted または typed terminal rejection を返す。
  - [x] source / tool / capacity / reservation / demand の代表 fixture が表示分類へ到達する。
  - [x] no wheelbarrow / 全台 PushedBy・使用中は current rebuild header で Missing / TemporaryContention を区別し、
  ...
  - [x] available wheelbarrow 0 の診断のために request / source / vehicle 全件 scan を追加せず、既存の全車 query の
    `is_empty()` または owner count を使い、未走査 request record を seed しない。
  - [x] arbitration rebuild がない frame は直前 snapshot を保持し、request removal / world replacement / revision mismatch は
    stale reason を利用せず Pending になる。
  - [x] ある Familiar / producer の rejection 後に別 Familiar / producer が submitted なら、TaskWorkers 0 は Pending、
    TaskWorkers > 0 は Working となり blocker を表示しない。
  - [x] lower-ranked、candidate universe 外、producer 未評価 task は reason vote があっても Pending のままである。
  - [x] TaskArea / Yard / `ManagedTasks` の異なる Familiar 間で、実 candidate 集合にない `NotApplicable` は
  ...
  - [x] Soul / source 候補数を変えても producer × applicable evaluator 1 vote の代表理由が安定する。
  - [x] worker spawn / sort 順を変えても同じ Familiar-local terminal outcome になり、raw rejection 件数を票にしない。
  - [x] general / blueprint producer の submitted 後に worker assignment が成立しない、または同 frame に解除された場合、
    `TaskWorkers` 0 を Working / Blocked と誤表示せず Pending にする。
  - [x] `PathSearchResult::Deferred` を `Unreachable` へ写像するコードがない。
  - [x] world replacement で Blueprint producer record / cycle header と wheelbarrow arbitration record / header の
    旧 Entity が 0 件になる。
  - [x] 列挙済みの Move false-positive 修正を除き、assignment request、reservation shadow、candidate /
...
- 変更ファイル:
  ...
  - `crates/bevy_app/src/plugins/logic.rs`
- 完了条件:
  - [x] nested Button がなく、action click で focus click が重複発火しない。
  - [x] stale Entity / Designation、capability 変更、pause / capture を apply 時に drain + 安全に拒否し、
    unpause 後の遅延適用と confirmation 持越しがない。
  - [x] 新規 manual Chop / Mine だけ provenance marker を保存し、legacy / load 後 auto / unknown task は read-only。
  - [x] priority は許可 task だけ変更し、Blueprint には表示せず、save/load 後も既存 `Priority` 値を保持する。
  - [x] generic cancel は `Priority` / provenance を残さず、manual request は lifecycle owner の typed API を通って
    worker / reservation / pinned source を残さない。anchor cleanup と UI cancel が同じ close primitive / 除去集合を使う。
  - [x] Blueprint cancel は全 Soul の `AssignedTask` payload から Build / HaulToBlueprint /
  ...
  - [x] Floor / Wall tile と関連 material request の双方から site-wide cancel でき、資材返却 / request / tile /
    WorldMap 契約を通る。save/load 後に target marker がなくても `TransportRequest.kind + anchor` から解決でき、
    stale index / 0 tile でも永久 pending にならない。
  - [x] Move / auto producer task を誤って変更・despawn できない。
  - [x] owner cancellation set + ApplyDeferred が production schedule で Familiar Perceive より前に実行され、
    TransportRequest Perceive / Decide と Soul Perceive も cancel 済み状態だけを読み、対象 request を同 frame に再生成しない。
  - [x] blocker 更新は notification を発行せず、操作 1 回が bounded notification 1 件以下になる。
...
- 変更内容:
  ...
  - 同一 schema / fixture で dashboard hidden / visible の counter と frame cost を比較する要件を性能フォローアップへ移管する。
  ...
  - 関連提案の A3 実装状態を更新し、受入後に本計画を archive する。
- 変更ファイル:
...
- 完了条件:
  ...
  - [x] load 後に旧 Entity の diagnostics / selection / confirmation が残らない。
  - [x] `PlayerIssuedDesignation` / priority の save 往復と、diagnostics の非永続・再評価を同時に確認できる。
  - [x] UiIntent / action outcome の未読 buffer と全 task-list dynamic child が reset 境界内で空になる。
  - [x] 提案 A3 の機能受入条件を自動テストと R01 / R02 へ対応付け、定量性能受入を独立計画へ移管できる。
...
T01〜T10 と T12 を自動化済みで、T11 は手動確認へ戻さず性能フォローアップへ移管した。
...
R01 / R02 は `2026-07-20` のユーザー受入で完了した。R03 は性能計測基盤 T11 と同じ独立計画へ移管し、
A3 のクローズ条件には含めない。
...
- 進捗: `100%`（A3 機能・自動回帰・恒久文書・R01 / R02 受入・archive 完了）
- 完了済みマイルストーン: `M0`〜`M5`
- A3 の残作業: なし。T11 / R03 は独立した性能フォローアップへ移管済み。
...
- [x] M0〜M5 がすべて完了
- [x] 提案 A3 の機能受入条件を自動テストと R01 / R02 で満たし、定量性能検証 T11 / R03 を独立計画へ移管する
- [x] coverage 不足を blocker とする false positive が 0 件
- [x] 非 global task で別 TaskArea の `NotApplicable` evaluator を待つ永久 Pending と、global Build / Yard-owned の
  evaluator を分母から落とす false blocker が 0 件
- [x] applicable producer 未評価と raw branch count による false blocker が 0 件
- [x] worker / source 件数・spawn 順に依存する Familiar-local vote、lease 不在 1 bit / 全台 PushedBy による誤分類、
  submitted を accepted と扱う false Working / Blocked が 0 件
- [x] dashboard state を AI system parameter にせず追加探索を呼ばない構造を保証し、mode 間 counter の定量検証を独立計画へ移管する
- [x] provenance 不明 / auto task を priority・cancel できず、許可 task の owner cleanup が完全
- [x] 影響ドキュメントが更新済み
...
| dashboard で候補評価・経路探索を増やさない | normal cycle の副産物、UI read-only | A02 / A03 / T12。T11 の定量比較は性能フォローアップ | R02 受入済み。R03 は性能フォローアップ |
...
- [x] 現在の early return 台帳に未分類の `None` / `false` がない。
- [x] coverage incomplete / zero rejection を Blocked にしていない。
- [x] 全 applicable producer が complete でない task を Blocked にしていない。
- [x] scope 名から coverage 分母を推測せず、global Build / Yard-owned を含む実 candidate 集合から
  task / producer ごとの applicable evaluator と `NotApplicable` を区別している。
- [x] raw worker / source branch count でなく producer × applicable evaluator 1 terminal vote を使っている。
- [x] Familiar-local reducer が submitted -> partial -> zero worker -> reason presence + fixed precedence を実装し、
  worker / source 件数・spawn / sort 順に依存していない。
- [x] submitted でない bool success を肯定証拠にしていない。
- [x] submitted を accepted と扱わず、current `TaskWorkers` なしを Pending にしている。
- [x] wheelbarrow lease 不在を直接分類せず、current arbitration snapshot の typed outcome と全車存在 header を使い、
  全台 PushedBy / 使用中を Missing と誤分類していない。
- [x] `Deferred` と `Unreachable`、距離制限と connectivity false を混同していない。
- [x] dashboard open / filter state が AI system parameter に入っていない。
- [x] hot path に String、reason Vec、履歴 queue、UI Query、追加 A* がない。
- [x] volatile input 1 件で全 task を無条件に永久 Pending にしていない。
- [x] semantic generation は owner crate 公開値 + root bridge で更新し、依存方向を逆転していない。
- [x] task-local revision sync が named `AutoGatherFlush -> TaskRevisionSync -> Delegation` seam に入り、
  anonymous `ApplyDeferred` との偶然の順序に依存していない。
- [x] cycle ID / revision だけで UI rebuild していない。
- [x] row focus Button の descendant に action Button を置いていない。
- [x] capability を cached UI 値だけで信頼していない。
- [x] manual Chop / Mine は durable positive provenance だけを許可し、marker 不在を manual と推定していない。
- [x] Blueprint priority を許可せず、cancel は全 Soul の direct `AssignedTask` payload、related request、refund、
  WorldMap を含む owner lifecycle を通る。
- [x] Move / auto producer へ generic cancel / priority mutation を適用していない。
- [x] ManualTransportRequest cancel と anchor cleanup が `hw_logistics` owner の同じ typed close primitive を使っている。
- [x] Floor / Wall は site-wide cancellation と表示し、load 後も request kind + anchor から site を解決している。
- [x] owner cancellation set + ApplyDeferred を Familiar Perceive より前に固定している。
- [x] Relationship target を直接書き換えていない。
- [x] world replacement 後に旧 Entity 参照が残らない。
- [x] capture / pause 中も action Message を drain し、confirmation を持ち越していない。
- [x] blocker state を A2 notification history へ周期送信していない。
- [x] current code / tests / durable docs が同じ owner と system order を説明している。
...
| `2026-07-20` | `Codex` | ユーザー受入により R01 / R02 を完了。未計測の T11 / R03 は独立性能計画へ移管し、A3 を完了・アーカイブ |
