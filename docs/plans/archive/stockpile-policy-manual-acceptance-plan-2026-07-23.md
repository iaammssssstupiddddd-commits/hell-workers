...
実装設計と自動回帰の完了記録は
`docs/plans/archive/stockpile-policy-plan-2026-07-20.md` と
`docs/plans/archive/stockpile-resource-checklist-plan-2026-07-24.md` を正本とし、本書から履歴を書き換えない。
...
- 全選択・全解除・全9資材チェックリスト、複数許可集合、priority、target、export、Drainingの表示と実挙動。
...
| 確認 | 結果 | 証跡 |
| --- | --- | --- |
| workspace品質ゲート | 合格 | `2026-07-24`: `python3 scripts/dev.py verify` |
...
| 複数資材acceptance集合、静的9行UI、range patch、save round-trip | 合格、自動のみ | `2026-07-24`: `hw_logistics`、`hw_ui`、`bevy_app` focused test |
| world replacement時のInfoPanel root即時非表示 | 合格、自動のみ | `2026-07-24`: 実Nodeの`Display::Flex -> None`回帰、production hook登録回帰、`hw_ui` 57件、`bevy_app` save 51件 |
...
| B1-R03 | 単一編集 | 合格（旧cycle UI） | `Any -> Only(Wood)`、`Normal -> High`、`Target 10 -> 9`、`Export On -> Off` をpointer操作で反映。checklist版はB1-R14で合格 |
| B1-R04 | F5/F9方針round-trip | 合格（旧cycle UI） | save本文とF9後の再選択の両方で `Only(Wood) / High / 9 / Off` を確認。複数集合はB1-R15で合格 |
| B1-R05 | world replacement後の情報パネルreset | 合格 | ユーザー実機でF9成功直後に旧情報パネルが消え、Stockpile再選択後に保存済みチェック状態、priority、target、exportが復元されることを確認 |
...
| B1-R14 | 資材チェックリストの表示と直接操作 | 合格 | ユーザー実機で `Allow All` / `Clear All`、全9資材の2列配置、個別 `[x] / [ ]`、Wood+Rock複数選択、全解除時の `Disabled` を確認。1280x720、UI Scale 1.25でも縦scrollで全操作へ到達 |
| B1-R15 | 複数集合の矩形適用とF5/F9 round-trip | 合格 | ユーザー実機で複数セルへのWood+Rock集合コピー、Toast対象件数、F5/F9後の全9チェック状態を確認 |

### 3.4 B1-R05の原因と修正

再現手順:

1. 管理対象Stockpileを選び、チェックリストをWoodだけON、`High / 9 / Off` でF5保存する。
2. live値だけをRockだけONに変え、`Draining` 表示にする。
...
- Stockpileを再選択すると、保存済み「WoodだけON / High / 9 / Off」を表示する。

実結果:

- loadとrehydrateは成功し、再選択後の方針値も保存内容どおりだった。
- 再選択前は旧「RockだけON / Draining」パネルが残り、別entityを選ぶと初めて更新された。
...
- `hw_ui::reset_for_world_replace` が `With<InfoPanel>` の静的rootをreplace phase内で同期的に
  `Display::None`へ戻す。描画cacheの特殊値化や次frameの強制再描画は行わない。
- `world_replace_reset_hides_visible_info_panel_root_immediately` は`app.update()`を挟まず、
  表示中の実Nodeがreset呼び出し直後に非表示になることを固定する。
- `registered_hw_ui_reset_hides_visible_info_panel_root` はrootのproduction UI plugin登録から
  `LoadResetRegistry`を実行し、`hw-ui` hookが同じ実Nodeを非表示にすることを固定する。
- `cargo test -p hw_ui --lib --locked` 57件と
  `cargo test -p bevy_app@0.1.0 --lib systems::save --locked` 51件が合格した。
- `2026-07-25` のユーザー実機再確認で、F9成功直後の旧情報パネル消去と、
  再選択後の保存済みチェック状態、priority、target、exportの復元を確認した。
- 以前観測したdespawn済みEntity warningは今回の再受入で再現報告がないため、
  B1-R05とは分離し、再発した場合だけ別件として発行元を切り分ける。
...
- 変更ファイル:
  ...
  - 本書と関連恒久文書
- 完了条件:
  - [x] 表示中の `InfoPanelRoot` がreset直後に `Display::None` となる自動回帰を追加する。
  - [x] F9成功直後に旧情報パネルが消える。
  - [x] 再選択後に保存済み方針値を表示する。
  - [x] load後に観測したdespawn済みEntity warningの再現有無を記録し、再発時は別件として扱う。
  - [x] reset修正の影響範囲に応じて、選択/editorとsave/loadの実機回帰を確認する。
...
- 変更内容: cycle UIを置換した全選択・全解除・個別toggle・複数集合・矩形コピー・save/loadを実機で確認する。
- 変更ファイル:
  - `docs/plans/stockpile-policy-manual-acceptance-plan-2026-07-23.md`
- 完了条件:
  - [x] 全9資材と全選択・全解除が1366x768内で操作できる。
  - [x] 1280x720、UI Scale 1.25でも縦scrollで下段のTarget、Priority、Export、範囲適用へ到達できる。
  - [x] Wood+RockだけをONにでき、Boneを含む他資材はOFF表示になる。
  - [x] 複数集合を矩形へ適用し、Toast件数と対象セルのチェック状態が一致する。
  - [x] F5/F9後も複数集合の全チェック状態が一致する。
- 検証:
  - 下記B1-R14、B1-R15の実機scenarioを使用する。
...
- 手動確認シナリオ:
  1. **選択/editor**: `Zones -> Stockpile` で管理セルを配置し、itemが同位置にあってもStockpile editorを開く。
     `State / Stored / Incoming / Accepted Resources / Target / Priority / Export / Apply Policy to Area` と、
     `Allow All` / `Clear All` / 全9資材が画面内に収まることを確認する。1280x720、UI Scale 1.25でも
     縦scrollで全操作へ到達できることを確認する。
  2. **資材チェックリスト**: `Clear All` 後にWoodとRockをONにし、2行だけが `[x]`、残りが `[ ]` になることを確認する。
     全解除時は `State: Disabled` となり、Wood/Rockは空セルの搬入候補になり、Boneは拒否されることを確認する。
  3. **複数セルrange**: 2個以上の管理セルを作り、Wood+Rock集合を矩形適用する。toastの `Changed / already matched /
     skipped / duplicate` と実セル数を照合する。Escape取消、drag中のpause/overlay、camera非移動も確認する。
  4. **Draining**: Wood入りdonorと空きのあるreceiverを同じownerに用意し、donorのWoodをOFF、RockをONにする。
     Woodを削除せず新規搬入を止め、consolidationで搬出し、空になった後にRockを受けることを確認する。
  5. **in-flight**: `Incoming > 0` を確認してpauseし、targetを `Stored` 以下へ下げて再開する。
     committed搬送は安全に完了し、その後の新規搬入が止まることを確認する。
  6. **save/load**: Wood+RockだけをONにしてF5保存し、live値を変えてF9する。確認dialog、load結果通知、
     旧panelの即時消去、再選択後の全9チェック状態を順に確認する。
  7. **特殊設備**: Tank、Mixer、`BucketStorage` を選択し、通常Stockpile policy editorが表示されないことを確認する。
- パフォーマンス確認:
...
- 進捗: `自動確認 100% / 旧cycle UI実機確認 100% / checklist再受入 100% / 15項目中 合格14・自動のみ1`
- 完了済みマイルストーン: `M1`、`M2`、`M3`、`M4`
- 未着手/進行中: `なし`

### 次のAIが最初にやること

1. 本計画に残作業はない。B1の受入記録としてarchive状態を維持する。
2. despawn済みEntity warningが再発した場合だけ別件で発行元を切り分け、B1-R05の表示resetと混同しない。

### ブロッカー/注意点

- B1のpolicy保存値はround-tripしており、B1-R05の旧panel表示resetもコード・自動回帰・実機再受入まで完了した。
  schema移行問題として扱わない。
- 旧cycle UIのB1-R03〜R04は履歴として保持し、合格済みchecklist版のB1-R14〜R15を現在受入の正本にする。
...
- B1 checklist実機受入: `2026-07-24 / pass / B1-R14〜B1-R15`
- B1-R05 focused test: `2026-07-24 / pass / hw_ui 57 tests、bevy_app save 51 tests`
- B1-R05実機再受入: `2026-07-25 / pass / F9直後の旧パネル消去、再選択後の保存値復元`
- 最終workspace品質ゲート: `2026-07-25 / pass / python3 scripts/dev.py verify`
- 未完了: `なし`

### Definition of Done

- [x] B1-R05が修正され、実displayを含む回帰testがある
- [x] B1-R05のF9実機再受入が完了
- [x] B1-R06〜B1-R12の残件が合格、または自動専用の根拠が記録済み
- [x] B1-R14〜B1-R15のchecklist実機受入が完了
- [x] 影響ドキュメントが更新済み
...
| `2026-07-24` | `Codex` | cycle UIを全選択・全解除・9資材チェックリストへ置換。複数集合の自動回帰を追加し、B1-R14〜R15を実機再受入として追加 |
| `2026-07-24` | `User / Codex` | ユーザー実機でチェックリスト表示・操作、複数集合の矩形適用、F5/F9 round-tripを確認し、B1-R14〜R15とM4を完了。残るblockerをB1-R05だけに更新 |
| `2026-07-24` | `Codex` | B1-R05の原因をInfoPanel rootの同期非表示漏れへ確定。reset hookと実Node回帰を追加し、コード修正・自動確認済み、F9実機再確認待ちへ更新 |
| `2026-07-25` | `User / Codex` | B1-R05を実機再受入。F9直後の旧パネル消去と再選択後の保存値復元を合格とし、全マイルストーン完了で本計画をarchive |
