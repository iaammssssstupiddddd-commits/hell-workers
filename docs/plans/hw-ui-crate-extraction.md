...
### M2: list 汎用ロジック抽出 (resize, minimize, tree_ops)
...
**変更内容:**
1. `crates/hw_ui/src/list/` に `tree_ops.rs`, `resize.rs`, `minimize.rs` 追加
2. `EntityListMinimizeState`, `EntityListResizeState` を hw_ui で定義（現在 root にある場合）
3. Root 側は re-export + プラグイン登録で使用
...
4. `list_legacy/`, `panels_legacy/` ディレクトリを削除
...
- 進捗: `0%`
- 完了済みマイルストーン: なし（SelectedEntity/HoveredEntity は既に hw_ui 抽出済み）
- 未着手: M1-M6
...
- `list/` (resize, minimize, tree_ops, DragState, visual)
...
- [ ] `include!` マクロが `src/interface/ui/` から完全に消滅
- [ ] `UiSetupAssets` という名前が codebase に残っていない
- [ ] `cargo check` が成功
- [ ] ゲーム起動で UI 動作確認済み
