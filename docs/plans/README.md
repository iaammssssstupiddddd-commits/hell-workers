...
`docs/plans` の文書ステータス一覧（更新日: 2026-09-25）。
...
| [3d-rtt/single-scene-light-field/02a-p02-acceptance-infrastructure-plan-2026-08-12.md](3d-rtt/single-scene-light-field/02a-p02-acceptance-infrastructure-plan-2026-08-12.md) | Completed | P02 formal / native受入を実行可能にする基盤整備計画。 |
| [3d-rtt/single-scene-light-field/03-indoor-light-domain-core-plan-2026-08-03.md](3d-rtt/single-scene-light-field/03-indoor-light-domain-core-plan-2026-08-03.md) | Completed | 固定精度のradial field、遮光grid、supercover LOSを`hw_infra`へ実装するP03。 |
...
| [orca-abcd-workflow-expansion-plan-2026-09-25.md](orca-abcd-workflow-expansion-plan-2026-09-25.md) | Active — 第1実装batch完了・実runtime受入待ち | 自然文受付、状態パネル、自動loop、自己修復、A/B並列計画、統括判断によるLinear課題発行、GitHub/CI連携を段階導入する上位計画。 |
...
| [orca-parallel-development-plan-2026-09-20.md](orca-parallel-development-plan-2026-09-20.md) | In Progress — 固定受付、Linear-linked worktreeの可視統括、日本語A/B/review tab、実案件の編集→検証→固定review→統合→最終reviewを受入。無介入cold-startと稼働中中断復旧を残す | Linear受付・Orca実行基盤の統合。既存guard/session再利用、段階移行と限定検証 |
| [orca-ui-lifecycle-plan-2026-09-23.md](orca-ui-lifecycle-plan-2026-09-23.md) | In Progress — 固定受付、実案件の実装・固定review・統合、承認後role tab整理、通常desktop起動まで受入。新規案件の無介入cold-start計測と稼働中agentの中断・再起動復旧が残る | Orca UI・受付・終了ライフサイクル統合計画 |
