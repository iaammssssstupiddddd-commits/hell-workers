...
**基本方針**:
- キャラクター（Soul・Familiar）は GLB モデル + ボーンアニメーション（Bevy AnimationGraph）で Camera3d に直接レンダリングする（`character-3d-rendering-proposal-2026-03-16` 採用）
- スプライトシート・ビルボード・プロキシ並走は Phase 3 で廃止する。Phase 2 の既存スプライト（`soul.png` 等）はプロキシ用に維持するが新規整備は行わない
...
**完了条件**:
- [ ] キャラクタースプライトの方向数が確定している
- [ ] アウトライン受入基準（線幅・揺らぎ・色・閾値）が数値または比較サンプルで記述されている
...
> **ブロック先**: MS-P3-Pre-D（Character GLB PoC）・MS-3-1（CharacterMaterial 本実装）
...
Familiar も同様のパイプラインで制作するが、Soul の PoC 完了後に着手する。

**完了条件**:
- [ ] P0 クリップ（Idle・Walk）が AnimationGraph で再生できる（コード側 MS-3-Char-A と同時検証）
...
> **ブロック先**: MS-3-Char-B（face_billboard_system + 顔アトラス統合）
...
- [ ] `CharacterMaterial.face_uv_offset` で表情が切り替わる（MS-3-Char-B と同時確認）
...
| MS-Asset-Char-GLB-A | MS-P3-Pre-D・MS-3-1 | Character GLB がないと CharacterMaterial の目視確認ができない |
| MS-Asset-Char-GLB-B | MS-3-Char-A | AnimationGraph 実装の視覚確認に必要 |
| MS-Asset-Char-Face | MS-3-Char-B | face_billboard_system + 顔アトラス統合の目視確認に必要 |
