...
MS-WFC-1 完了後〜MS-WFC-4 までの間、**`bevy_app::compute_site_yard_layout()`** と **`hw_world::AnchorLayout::fixed()`** が同じ数式で矩形を返すことになる。定数（`SITE_WIDTH_TILES` 等）変更時に **片方だけ更新**されると、spawn とマスクがずれる。

- **中長期の方針**: 唯一のソースは **`hw_world::AnchorLayout`**（またはそれを返す `hw_world` の単一関数）とし、**MS-WFC-4** で `compute_site_yard_layout` を **`AnchorLayout` から構築**するか、共有ヘルパへ寄せて **二重定義を解消**する。
- この MS では **両方に同じ式をコピー**してよいが、変更時は **両ファイルを同時に直す**こと（または TODO コメントで 3.4 を参照）。
...
7. `layout.rs` に `INITIAL_WOOD_POSITIONS` / `ROCK_POSITIONS` / `TREE_POSITIONS` の廃止コメントを追加する
8. `initial_spawn/mod.rs` に `INITIAL_WHEELBARROW_PARKING_GRID` の廃止コメントを追加する
9. `cargo check --workspace` と `cargo clippy --workspace` を通す
...
§3.4 を参照。MS-WFC-4 まで `compute_site_yard_layout` と `AnchorLayout::fixed()` が併存する間は、定数変更時の **同時更新**を徹底する。
