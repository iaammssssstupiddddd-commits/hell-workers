...
- `CharacterMaterial` 実装・`CharacterHandles` リソース定義・Soul/Familiar GLB スポーン
- `SectionMaterial` 実装・`Building3dHandles` の `SectionMaterial` 移行
- AnimationGraph + `SoulAnimState` 実装・タスク状態連動（MS-3-Char-A）
- Familiar GLB 対応・アニメーションクリップ全統合（MS-3-Char-B）
...
| CharacterMaterial | 未実装 | キャラクター GLB が SectionCut クリップ・AlphaMode::Blend の恩恵を受けられない |
...
- キャラクターには `CharacterMaterial`（`AlphaMode::Blend`）を独立定義する。`SectionMaterial` とクリップ平面の WGSL モジュール（`section_clip.wgsl`）を共有する
- `SoulProxy3d` / `FamiliarProxy3d` / `Billboard` コンポーネントは Phase 3 で廃止し GLB エンティティへ置き換える
...
Phase 3 キャラクター（M-3-1 完了後・GLB アニメクリップ並走）
  ...
  M-3-Char-B: Familiar GLB + アニメーションフルセット      ← M-3-Char-A 完了後
...
Phase 3 後半
  M-3-8: 2D スプライトインフラ廃止（MS-3D）               ← M-3-7 + M-3-Char-B 完了後
...
### M-3-1: Camera3d 斜め角度適用 + CharacterMaterial 本実装

> **依存**: M-Pre3・M-Pre4 完了
> **根拠**: `character-3d-rendering-proposal` §3

**やること**:
1. `camera_sync.rs` の `sync_camera3d_system` を斜め角度用変換式に更新（`VIEW_HEIGHT`・`Z_OFFSET` 定数適用）
2. `assets/shaders/common/section_clip.wgsl` を新規作成（`SectionMaterial` と `CharacterMaterial` で共有するクリップ平面モジュール）
3. `hw_visual/src/material/character_material.rs` を新規作成（`CharacterMaterial`・`AlphaMode::Blend`・`CharacterHandles`）
4. `assets/shaders/character_material.wgsl` を新規作成（`#import "common/section_clip.wgsl"` + キャラクター固有処理）
5. `MaterialPlugin::<CharacterMaterial>` を `HwVisualPlugin` に登録
6. `building_completion/spawn.rs` の `SoulProxy3d` / `FamiliarProxy3d` スポーンを GLB ベースの実装へ置き換え
7. Camera2d 側の Soul / Familiar Sprite を削除
8. `SoulProxy3d` / `FamiliarProxy3d` / `Billboard` コンポーネント定義を削除

**変更ファイル**:
- `systems/visual/camera_sync.rs`
- `assets/shaders/common/section_clip.wgsl`（新規）
- `hw_visual/src/material/character_material.rs`（新規）
- `assets/shaders/character_material.wgsl`（新規）
- `hw_visual/src/visual_handles.rs`（`CharacterHandles` 追加・`Building3dHandles` からキャラクターハンドルを分離）
- `hw_visual/src/visual3d.rs`（`SoulProxy3d` / `FamiliarProxy3d` / `Billboard` 削除）
- `hw_visual/src/lib.rs`
- `building_completion/spawn.rs`（GLB スポーンに置き換え）
- Soul / Familiar の Camera2d Sprite spawn 箇所
...
- [ ] `sync_section_cut_to_character_materials` が `CharacterHandles` を参照して動作する
...
### M-3-Char-B: Familiar GLB 対応 + アニメーションフルセット

> **依存**: M-3-Char-A 完了・MS-Asset-Char-GLB-B 完了（P1 クリップ）
> **根拠**: `character-3d-rendering-proposal` §3
...
4. MS-Asset-Char-Face（顔テクスチャアトラス）を `CharacterMaterial.face_uv_offset` に統合する

**変更ファイル**:
- `hw_visual/src/anim/familiar_anim.rs`（新規）
...
> **依存**: M-3-7 完了・M-3-Char-B 完了（Familiar を含む全キャラクターが GLB 移行済みであること）
...
- 進捗: `0%`（計画ドラフト完了）
- 完了済みマイルストーン: なし
- 今すぐ着手可: M-Pre1・M-Pre2・M-Pre3（並走可）
...
3. M-Pre1（`WgpuFeatures::CLIP_DISTANCES` 確認）から着手する
4. M-Pre2（RtT 基盤整備）を M-Pre1 と並走させる
5. M-Gate（MS-2C 目視確認）を済ませてから M-Pre3 に移行する

### ブロッカー/注意点

- M-Pre1 が失敗した場合（`CLIP_DISTANCES` 非対応）は `section-material-proposal` §3.3 の技術設計を再検討すること
- Camera3d の `RenderTarget::Image` 型は `Handle<Image>` → `ImageRenderTarget` の `From` 実装で対応済み（MS-1B で確認済み）
- `SoulProxy3d` / `FamiliarProxy3d` / `Billboard` / `CharacterBillboardMaterial` はすべて廃止対象。M-3-1 で削除する
...
- M-Pre4（Character GLB PoC）は MS-Asset-Char-GLB-A（Soul GLB 配置）完了後でないと着手できない
...
- `hw_visual/src/visual3d.rs`（`SoulProxy3d` / `FamiliarProxy3d` 定義・M-3-1 で削除対象）
...
- 最終 `cargo check`: `2026-03-16` / pass（Phase 2 実装完了時点）
- 未解決エラー: なし
