...
- 進捗: 実装 `88%`。M0のgeometry/全16mask、M1のtechnical candidate、M2の3D/2D adapterとPostUpdate境界、M3aを完了。承認済みgeneration 5 bytesをclean subject `49c43e0f`のgeneration 6 authoring finalへ封印し、preview無効のisolated candidateでEW/NS 6状態の実機visual legまで通過した。`door-art-v1-behavior`の候補identity・production mesh/material・本番producer・load rebind用fail-closed runnerも実装済み。
- 次の作業: 専用clean validation worktreeで`door-art-v1-behavior`を採取し、quality/DPI、Door density Capture/Memoryを続けてDoor単独M3bを閉じる。その後に共通J1、releaseを行う。
...
- behavior受入基盤: `2026-09-07`に`door-art-v1-behavior`を追加。既存P08の7 behavior caseを変えず、専用opt-in時のみ`IsolatedCandidate` identity、semantic state対応のproduction mesh、共有material、production 1 / fallback 0を各processでfail-closed検証する。case別statusは共通nonceへ結び、既存P08 data schema外へ保存するためhistorical artifact契約は不変。runner self-testとprofiling feature compileはpass、正式実機採取は次項。
- ブロッカー: ArtPreview承認と正式candidate visual legは解消。実producerによるSoul通過・施錠・load、quality/DPI、Capture/Memory、Wall単独M3との共通J1が未完であり、releaseは行わない。
