...
- 解決したい課題:
  - `src/interface/selection/building_place.rs` が入力処理、companionフロー、配置可否判定、spawn処理を同居しており変更影響が広い。
  - `src/interface/selection/floor_place.rs` で floor/wall の検証ロジックが重複し、エラーメッセージ更新時に差分ズレが起きやすい。
...
- `src/interface/selection/building_place.rs` のモジュール分割。
- `src/interface/selection/floor_place.rs` のモジュール分割。
...
1. `building_place.rs` と `floor_place.rs` を責務区分でマーキングし、M1の移動単位を確定する。
...
- `src/interface/selection/building_place.rs`
- `src/interface/selection/floor_place.rs`
- `src/interface/selection/mod.rs`
...
- 最終 `cargo check`: `2026-02-25` / `not run`
- 未解決エラー: `N/A`（未実装のため）

### Definition of Done

- [ ] `M1` `M2` `M3` `M4` の完了条件を満たす
- [ ] 配置フローの主要シナリオで回帰がない
- [ ] `cargo check` が成功する
