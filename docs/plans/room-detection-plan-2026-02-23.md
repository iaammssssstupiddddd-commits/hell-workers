...
1. 手動シナリオ（部屋成立/破壊/ドアなし/仮壁）をプレイ確認する。
2. 必要であれば `ROOM_DETECTION_COOLDOWN_SECS` / `ROOM_VALIDATION_INTERVAL_SECS` を調整する。
3. Room効果（温度・モラル）実装時に `RoomTileLookup` を参照先として利用する。
...
- 最終 `cargo check`: `2026-02-23` 実行、成功
- 未解決エラー: なし

### Definition of Done

- [x] Room検出ロジック（dirty追跡 + Flood-fill + 検証）が動作する
- [x] Roomオーバーレイ表示が同期される
- [x] 関連ドキュメント（提案/建築仕様/計画一覧）が更新される
- [x] `cargo check` が成功する
...
| `2026-02-23` | `Codex` | M1〜M6 実装完了に合わせてステータス更新 |
