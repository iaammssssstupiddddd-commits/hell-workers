...
| 状態 (State) | 説明 |
| :--- | :--- |
| **`Idle`** | `Idle` 命令中で、有効な所属 Soul がまだいない状態。その場に留まります。 |
...
- **リクルート遷移**: 命令種別にかかわらず、所属 Soul が1体以上いれば空き枠が残っていても `Supervising` に移行します。判断周期ごとに追加募集を先に試すため、既存メンバーの監視と上限までの募集を両立します。遠方候補が見つかった場合だけ `Scouting` を優先します。
- **Idle コマンド時のリクルート**: `FamiliarCommand::Idle`（TaskArea なし含む）でも `max_controlled_soul` を目標にリクルートを継続し、既存メンバーがいれば監視します。

## 3. 監視ロジック (Supervising)

監視モードでは、部下との「適切な距離」を保ちながら行動します。分隊が満員である必要はなく、命令が `Idle` でも有効な所属 Soul が1体以上いれば監視対象になります。
...
- **部下側の追従停止**: 待機中の部下は使い魔の `command_radius` 外では使い魔を追い、範囲内へ入ると現在位置を `Destination` にして経路と計画メタデータを破棄します。これにより、停止直後のActor経路探索が使い魔への経路を再生成しません。
