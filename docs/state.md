...
書き込み元: `arch_category_action_system`（`crates/bevy_app/src/interface/ui/interaction/systems.rs`）
...
- `crates/hw_core/src/game_state.rs` - PlayMode、Context定義
- `crates/bevy_app/src/main.rs` - State登録、OnEnter/OnExit
- `crates/bevy_app/src/interface/selection/` - Escキーによるキャンセル処理
- `crates/bevy_app/src/interface/ui/interaction/systems.rs` - ボタンによる状態遷移とモード表示更新
