...
- AreaEdit drag は元の `TaskArea` / `Destination` / `ActiveCommand` を復元する。Dream preview seed、Zone
  removal preview、building move の未確定 state も同じ cleanup で破棄する。
...
- `crates/bevy_app/src/input_actions/{bindings.rs,cancel.rs}` - context 別 Escape 解決と共通 owner cleanup
