...
| `crates/hw_ai/src/soul_ai/update/dream_update.rs` | dream 蓄積（起動中）・放出（睡眠中）・DreamQuality 判定 |
| `crates/hw_ai/src/soul_ai/update/rest_area_update.rs` | 休憩所での per-soul dream 放出 + 休憩更新 |
| `crates/hw_ai/src/soul_ai/update/vitals_influence.rs` | dream 乗算ストレス増加 |
| `crates/hw_ai/src/soul_ai/decide/idle_behavior/transitions.rs` | dream=0 ガード付き行動選択関数 |
| `crates/hw_ai/src/soul_ai/decide/idle_behavior/mod.rs` | dream=0 強制起床・wants_rest_area/休憩クールダウン ガード |
| `crates/hw_ai/src/soul_ai/decide/idle_behavior/motion_dispatch.rs` | 集会 Sleeping サブ行動 dream=0 チェック |
