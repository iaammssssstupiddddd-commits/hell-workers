...
獲得量の保留は Soul component ではなく `hw_visual::DreamPresentationLedger` が所有します。

## 3. Dream 蓄積・放出ロジック

### 3.1 蓄積：起きている間 (`slow_simulation_driver_system`)
...
### 3.2 放出：睡眠中 (`slow_simulation_driver_system` → `dream_update_step`)
...
### 3.3 放出：休憩所 (`slow_simulation_driver_system` → `rest_area_update_step`)
...
### 3.4 実移送量と presentation

- production は 100 ms 刻みの `slow_simulation_driver_system` が所有し、1 frame 最大5 stepを
  `fatigue → penalty → rest → dream → influence` の順で処理する。
- 各 step は `DreamPool.points` へ加算したものと同じ `actual_drain` を frame-local accumulator に記録し、
  driver 終了時に Soul ごと最大1件の `DreamTransferredVisualMessage` を発行する。
- Message は `amount`、drain時点の`DreamQuality`、Soul/RestArea sourceとfallback座標、
  producerがtransfer streamの終了を確定したことを示す`is_final`をsnapshotする。
  Visual側は現在のdream残量・relationship・drain rateから獲得量や終了状態を再計算しない。
- `hw_visual` の ingestion はcamera/UIの有無に関係なくMessageを読み、channel別の未配信量を
  `DreamPresentationLedger`へ保持する。world replacementではMessage bufferとledgerをclearする。
- `is_final`付きtransferは閾値未満のtailも即時flushする。明示的な終了を受け取らない場合は
  `DREAM_POPUP_INTERVAL`（0.5秒）の無通信後にtailをflushし、100 ms slow-step間の通常frameは
  transfer終了とみなさない。
- RestAreaのoccupant数ベースworld粒子はambient演出であり、Poolへ移送したDream量には数えない。
...
Dream メニューに keyboard shortcut はない。`D` は `PanCameraPlugin` の右パン入力として維持する。
ドラッグ中に Modal/Pause capture が始まった場合は `DreamPlanting(Some(_))` を `None` に戻して preview
seed を破棄する。release 済みの `pending_dream_planting` は確定済み request として維持する。
...
### 8.5 関連定数（`crates/hw_core/src/constants/dream.rs`）
...
| ファイル | 内容 |
| :--- | :--- |
| `crates/hw_core/src/soul.rs` | `DamnedSoul.dream`, `DreamQuality`, `DreamState`, `DreamPool` 等の基礎型定義 |
| `crates/bevy_app/src/entities/damned_soul/mod.rs` | Bevy への Reflection 登録・リソース初期化 |
| `crates/bevy_app/src/entities/damned_soul/spawn.rs` | Soul スポーン時の `DreamState::default()` |
| `crates/hw_core/src/constants/dream.rs` | Dream 関連全定数（蓄積・放出・ストレス・ビジュアル） |
| `crates/hw_core/src/events.rs` | `DreamTransferredVisualMessage` とdrain時点のsource / final snapshot |
| `crates/hw_soul_ai/src/soul_ai/update/slow_simulation.rs` | production slow-step順序、Soul単位のtransfer集約・発行 |
| `crates/hw_soul_ai/src/soul_ai/update/dream_update.rs` | dream 蓄積（起動中）・放出（睡眠中）・DreamQuality 判定 |
| `crates/hw_soul_ai/src/soul_ai/update/rest_area_update.rs` | 休憩所での per-soul dream 放出 + 休憩更新 |
| `crates/hw_soul_ai/src/soul_ai/update/vitals_influence.rs` | dream 乗算ストレス増加 |
| `crates/hw_visual/src/dream/gain_visual.rs` | always-on ingestion、durable ledger、popup/UI particleへのchannel別配信 |
| `crates/hw_soul_ai/src/soul_ai/decide/idle_behavior/transitions.rs` | dream=0 ガード付き行動選択関数 |
| `crates/hw_soul_ai/src/soul_ai/decide/idle_behavior/system.rs` | dream=0 強制起床・wants_rest_area/休憩クールダウン ガード |
| `crates/hw_soul_ai/src/soul_ai/decide/idle_behavior/motion_dispatch.rs` | 集会 Sleeping サブ行動 dream=0 チェック |
| `crates/bevy_app/src/systems/dream_tree_planting.rs` | Dream 植林コアロジック |
