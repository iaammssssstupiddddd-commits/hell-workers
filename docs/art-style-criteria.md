...
Doorは高さ32 wu、左右jambの外端X=`±16`、前後Z=`±4.8`をWall portへ合わせた固定枠と、左右2枚の木製leafを持つ。上枠はY=`[14.4,16.0]`の1.6 wu、奥行7.2 wuとし、jambより細いシルエットを保つ。Closed / Open / Lockedは各1 node・1 mesh・1 primitiveの共有GLBで、triangle数は`204 / 204 / 216`、上限240。Openは枠を動かさず、左右leafと付属金具を同じ側へ同じ78°だけ対称に開く。各leafの上下2本の骨補強は扉面と同じ蝶番姿勢を取り、EWでは扉面と中央開口、NSでは補強線の向きから枠外へ張り出す扉面と開口を読める形にする。左右を反対側へ開く、または左右の開度を変えて識別性を作らない。Lockedだけが中央を跨ぐ骨の閂を持つ。

共有albedoは512×512・完全Opaque、emissive / normalなし。黒ずんだ厚板、風化した骨、少量の錆鉄をRough Vector Sketchの太いbaked lineworkで描く。EW/NS previewは同じClosed原本から59°正射影で256×256 RGBAへ固定レンダーし、pixel anchor `(128,192)`を64×64 logical canvasへ対応させる。generation 5 ArtPreviewはゲーム所有windowでユーザー承認済み。承認記録は候補manifestとcapture・review crop・status/ACKのhashへ結び、正式candidateの単独受入、Wall型枠とのjoint受入、releaseが終わるまで通常Doorはprocedural fallbackを使う。
