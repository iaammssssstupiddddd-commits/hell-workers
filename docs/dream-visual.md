...
> 今後拡張予定のため [`gain_visual.rs`](../crates/hw_visual/src/dream/gain_visual.rs) に独立モジュールとして配置しています。
...
**material は共有バケットプールを使う**（world-space 泡の §2 と同方針）。`DreamBubbleUiHandles` が起動時に `alpha(8) × mass(4) × color(2) × velocity(8)` の固定プール（`DreamBubbleUiMaterial` ハンドル群）を生成し、各 UI 泡・軌跡は `DreamUiMaterialBucket` コンポーネントで現在のバケットを保持する。毎フレームの物理演算結果はバケット値へ量子化され、バケットが変わったときだけ `MaterialNode` のハンドルを差し替える（`apply_ui_material_bucket`）。粒子ごとの `Assets<DreamBubbleUiMaterial>::add` / `get_mut` は行わない（毎フレーム・粒子単位の material dirty 化を避け、batchability を保つ）。`time` は material uniform ではなくシェーダー側の `globals.time` を参照する。
