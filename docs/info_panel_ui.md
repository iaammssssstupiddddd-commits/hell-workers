...
Familiar の `Open Operation` は InfoPanel 内の editor ではなく、コンテキストメニューから開く独立 overlay です。
受理時にクリック対象を latch するため、ダイアログを開いた後に `SelectedEntity` や
`InfoPanelPinState` が変わっても編集対象は切り替わりません。
