...
Soul行は最小32pxのコンパクトな2段グリッド、上下padding各1px・行間margin1px、本文14px、補助12pxを初期値とする。名前と数値の基準位置を揃え、長い名前は名前の列内で折り返す。氏名も共通UIフォントで表示する。

### ヘッダー
- タイトル: `Entity List`
- 右側に最小化ボタン（`-` / `+`）
- 最小化時はEntities/Tasks両本文と検索行を非表示化し、パネル高さをヘッダー相当へ縮小
- `LeftPanelMode` と `EntityListMinimizeState` を正本に、tab/minimize処理後の単一visibility systemが本文と検索行を切り替える
- Tasks中と最小化中はSoul検索を隠して入力focusを解除する。検索文字列はタブ往復で保持する
...
- `Scroll: Mouse Wheel` ヒント（必要時のみ表示）。本文外の通常レイアウト領域を確保し、最終行へ重ねない
...
- 上段に性別・名前・作業、下段に疲労・ストレス・Dream。現地ボタンは右端48px列で2段にまたがり中央揃え
