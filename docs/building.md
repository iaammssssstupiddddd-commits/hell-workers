...
| `VisualLayerKind` | `Building` エンティティの子として生成されるビジュアルレイヤー種別（`hw_visual::layer`）。`Floor / Struct / Deco / Light` のいずれかを持ち、`Sprite` と共にスポーンされる。Phase 2 以降で `RenderLayers::layer(1)` を付与するだけで3D側へ移行できる設計 |
