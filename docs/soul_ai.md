...
| 場所 | 担当システム |
|:--|:--|
| `crates/hw_ai` | `update/*`（vitals, fatigue, dream, rest_area, state_sanity, gathering_tick）/ `helpers/gathering` / `execute/designation_apply` |
| `src/systems/soul_ai` | `decide/*` / `execute/task_execution` / `execute/gathering_spawn` / `perceive/*` / `visual/*` |

root 側の shell には `WorldMap`・SpatialGrid・`GameAssets` を参照するシステムが残る。詳細は `docs/plans/hw-ai-crate-plan-2026-03-08.md` §M5 を参照。
