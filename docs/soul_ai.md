...
root 側の shell には `GameAssets`・`PopulationManager`・`task_execution`・visual/UI 依存を持つシステムが残る。
`helpers/gathering_positions`・`gathering_motion`・`decide/idle_behavior`・`decide/escaping`・`decide/gathering_mgmt`・`perceive/escaping` の純粋実装は `hw_ai` に移動済み。  
...
`gathering_spawn` は `hw_ai` 側の発生判定 (`gathering_spawn_logic_system`) と root 側の visual adapter (`gathering_spawn_system`) に分離されている。adapter 側は `GatheringSpawnRequest` 消費時に initiator の存在・idle 状態・ task/relationship 条件を再検証し、同一フレームで stale になった request を破棄する。  
