...
`helpers/work` は `is_soul_available_for_work` が `hw_ai` へ移され、`unassign_task` は root facade として残置される。低レベル cleanup 本体は `hw_soul_ai::soul_ai::helpers::work::cleanup_task_assignment` にあり、root 側が `OnTaskAbandoned` と `WorkingOn` の責務を確定する。
...
3. `helpers/work.rs::unassign_task`:
   `WorldMap`・`Visibility`・`Transform`・`WorkingOn` を扱う root facade。低レベル cleanup は `hw_soul_ai::soul_ai::helpers::work::cleanup_task_assignment` に委譲する。
