...
| 場所 | 担当システム |
|:--|:--|
| `crates/hw_ai` | `update/*`（vitals, fatigue, dream, rest_area, state_sanity, gathering_tick） / `helpers/gathering`, `helpers/gathering_positions`, `helpers/gathering_motion`, `helpers/work::is_soul_available_for_work` / `execute/designation_apply`, `execute/gathering_apply` / `decide/work::{auto_refine,auto_build}` / `decide/idle_behavior/transitions`, `decide/idle_behavior/task_override`, `decide/idle_behavior/exhausted_gathering`, `decide/escaping`, `perceive/escaping`, `decide/gathering_mgmt` |
| `src/systems/soul_ai` | `decide/idle_behavior/mod.rs`（idle_behavior_decision_system）/ `decide/drifting` / `execute/task_execution` / `execute/gathering_spawn` / `helpers/work.rs::unassign_task` / `perceive/*`（薄い re-export shell）/ `visual/*` |

root 側の shell には `WorldMap`・SpatialGrid・`GameAssets`・`RestArea` を参照するシステムが残る。
