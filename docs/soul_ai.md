...
| 場所 | 担当システム |
|:--|:--|
| `crates/hw_ai` | `update/*`（vitals, fatigue, dream, rest_area, state_sanity, gathering_tick）/ `helpers/gathering` / `helpers/gathering_positions` / `helpers/gathering_motion` / `execute/designation_apply` |
| `src/systems/soul_ai` | `decide/*` / `execute/task_execution` / `execute/gathering_spawn` / `perceive/*` / `visual/*` |

root 側の shell には `WorldMap`・SpatialGrid・`GameAssets` を参照するシステムが残る。
`helpers/gathering_positions` と `helpers/gathering_motion` は `hw_world::PathWorld` + `hw_world::SpatialGridOps` を引数に取る純粋 helper として `hw_ai` に移動済み。root 呼び出し側は `&WorldMap`（`PathWorld` impl）と `&SpatialGrid`（`SpatialGridOps` impl）を渡す。
