...
| `crates/bevy_app/src/systems/soul_ai` | `decide/drifting`（`PopulationManager` 依存のため root 残留） / `decide/idle_behavior/mod.rs`（薄い re-export shell） / `execute/task_execution/mod.rs`（薄い re-export shell） / `execute/task_execution/{common,context,handler,move_plant,types}`（薄い re-export shell） / `execute/task_execution/transport_common/*`（root 側互換 helper と `hw_jobs::lifecycle` re-export） / `execute/gathering_spawn`（visual adapter、`GameAssets` 依存） / `helpers/work.rs`（薄い re-export shell） / `perceive/*`（薄い re-export shell） / `visual/*` |
...
`decide/work/auto_refine.rs` と `decide/work/auto_build.rs` の実体と system 登録は `hw_soul_ai::SoulAiCorePlugin` 側へ移り、root 側は互換パス維持用の thin re-export のみを残す。  
...
`helpers/work` は `is_soul_available_for_work` と `unassign_task` の両方が `hw_soul_ai` にあり、root 側 `helpers/work.rs` は互換 import path の thin shell にとどまる。`cleanup_task_assignment` と `unassign_task` の両方を `hw_soul_ai::soul_ai::helpers::work` が所有する。
