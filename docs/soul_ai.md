...
| `crates/bevy_app/src/systems/soul_ai` | `decide/drifting`（`PopulationManager` 依存のため root 残留） / `decide/idle_behavior/mod.rs`（薄い re-export shell） / `execute/task_execution/mod.rs`（薄い re-export shell） / `execute/task_execution/{common,context,handler,move_plant,types}`（薄い re-export shell） / `execute/task_execution/transport_common/*`（root 側互換 helper と `hw_jobs::lifecycle` re-export） / `execute/gathering_spawn`（visual adapter） / `helpers/work.rs`（薄い re-export shell） / `perceive/*`（薄い re-export shell） / `visual/*` |
...
`apply_task_assignment_requests_system` の実装と system 登録責務は `hw_soul_ai::soul_ai::execute::task_assignment_apply` / `hw_soul_ai::SoulAiCorePlugin` 側に一本化されている。`task_execution_system` も `hw_soul_ai::soul_ai::execute::task_execution_system` が所有し、root 側の `crates/bevy_app/src/systems/soul_ai/execute/task_execution/mod.rs` は互換 import path の thin re-export にとどまる。`SoulAiPlugin` は system 本体を再登録せず、crate 側 system への `.after(...)` / `.before(...)` の順序参照だけを行う。移設済み system を root 側で再登録すると Bevy の schedule 初期化で ordering 対象が曖昧になり panic する。
