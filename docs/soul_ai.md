...
| 場所 | 担当システム |
|:--|:--|
| `crates/hw_ai` | `update/*`（vitals, fatigue, dream, rest_area, state_sanity, gathering_tick）/ `helpers/gathering` / `helpers/gathering_positions` / `helpers/gathering_motion` / `execute/designation_apply` / `execute/gathering_apply` / `decide/idle_behavior/transitions` / `decide/idle_behavior/task_override` / `decide/idle_behavior/exhausted_gathering` |
| `src/systems/soul_ai` | `decide/idle_behavior/mod.rs`（idle_behavior_decision_system）/ `decide/escaping・drifting・gathering_mgmt・work` / `execute/task_execution` / `execute/gathering_spawn` / `execute/idle_behavior_apply` / `execute/escaping_apply` / `perceive/*` / `visual/*` |

root 側の shell には `WorldMap`・SpatialGrid・`GameAssets`・`RestArea` を参照するシステムが残る。
`helpers/gathering_positions`・`gathering_motion`・`decide/idle_behavior` の純粋ヘルパー群は `hw_ai` に移動済み。root 呼び出し側は `&WorldMap`（`PathWorld` impl）と `&SpatialGrid`（`SpatialGridOps` impl）を渡す。
