...
| 場所 | 担当システム |
|:--|:--|
| `crates/hw_ai` | `update/*`（vitals, fatigue, dream, rest_area, state_sanity, gathering_tick） / `helpers/gathering`（`hw_core::gathering` の互換 re-export）, `helpers/gathering_positions`, `helpers/gathering_motion`, `helpers/drifting`, `helpers/work::is_soul_available_for_work` / `execute/designation_apply`, `execute/gathering_apply`, `execute/gathering_spawn` / `decide/work::{auto_refine,auto_build}`, `decide/idle_behavior/mod.rs`, `decide/escaping`, `perceive/escaping`, `decide/gathering_mgmt` |
| `src/systems/soul_ai` | `decide/drifting` / `decide/idle_behavior/mod.rs`（薄い re-export shell） / `execute/task_execution` / `execute/gathering_spawn`（visual adapter） / `helpers/work.rs::unassign_task` / `perceive/*`（薄い re-export shell）/ `visual/*` |
...
両型は `TaskArea`（root の `app_contexts` 依存）に強く結合しているため、現状 `hw_jobs` への移設が困難。
