...
2.  **既存コードの改修時**: 既存の直接依存（例: `hw_visual` → `hw_jobs` の直接参照）に触れる改修を行う際は、改修範囲内でミラーコンポーネントパターン（`hw_core::visual_mirror::*`）への切り替えを検討する。ただし、改修スコープを大幅に超える書き換えは強制しない。（`hw_visual` の主要な直接依存は 2026-03-15 にミラーコンポーネントへ移行済み。残存: `mud_mixer.rs` / `tank.rs` / `wall_connection.rs` の Building 系）
