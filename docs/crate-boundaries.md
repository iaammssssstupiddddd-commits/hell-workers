...
### 1.2 `hw_infra::lighting`のpure core境界
...
*   **Root 側の責務 (`bevy_app`):**
    *   production 構成では `HellWorkersGamePlugin` が各 Leaf クレートと root plugin を束ねて `App` に一意に追加する。binary `main.rs` は Window / Render / backend を設定してこの plugin を追加するだけに留める。
    *   `HellWorkersGamePlugin` が `GameSystemSet` 全体の実行順序（Input -> Logic -> Visual 等）を定義する。
...
### 3.4. Index-backed adapter の現行所有

| 機能 | model / pure rule | index adapter / metrics | production登録・root-only処理 |
|:---|:---|:---|:---|
...
UI hoverも同じ境界を使う。`hw_ui`はdomain-neutralな`HoverActionTarget`を読み、root adapterが
`BuildingCategory::Plant`等のdomain判定を行う。buttonの`Changed<Interaction>`は`ui_interaction_system`だけが
`UiIntent`へ変換し、ゲーム副作用はrootの`handle_ui_intent`へ一意に集約する。
