...
| `AssignedTask` | `apply_task_assignment_requests` (Execute) | `task_execution_system` (Execute) | 正常完了は `complete_task` → `TaskEndDisposition::Completed` で `OnTaskCompleted` 発火 |
...
- **フレーム内差分**: `ResourceReservationRequest` → `hw_logistics::apply_reservation_requests_system` で即時反映
...
| `clear_soul_assignment` | 専用 cancel が予約 cleanup 済み | disposition 次第 | 同上 |

`task_execution_system` は `end_disposition == Completed` のときだけ `OnTaskCompleted` を trigger する。
...
| `OnTaskCompleted` | `TaskEndDisposition::Completed`（`complete_task` 経由） | **やる気ボーナス付与**（Chop/Mine+2%、Haul+1%、Build系+5%）/ 音声 |
