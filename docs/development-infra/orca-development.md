...
## A〜D拡張の実装状況（2026-09-25）

[統合計画](../plans/orca-abcd-workflow-expansion-plan-2026-09-25.md)に基づき、host candidate
`2897e3a1`とOrca本体UI candidate `dd50afed`を実装した。復帰可能な配備、隔離actual-windowでの
受付表示・閉鎖、実Linear status/comment、GitHub Draft PR、重複抑止、誤状態write拒否、
試験専用branch mergeを受入済みである。稼働中TAK-14を止めないため、次回通常起動のcold-start確認だけを残している。
...
- 外部同期はoperation順のintent ledgerを正本とし、Linear/GitHub eventを未信頼の提案として扱う。
  executorは送信前のprovider read、送信後のexact read-back、`unknown`時の再送禁止を行う。
  Draft PRはremote base/head、tested/review SHA、固定reviewer、公開権限が一致するまで作成しない。
  Linear statusはteam stateのname/type/順序と非後退条件を照合し、誤指定を`blocked_policy`で保全する。
...
`/home/satotakumi/.local/opt/orca-ide/ui-dd50afed`で、desktop iconとCLIは次回通常起動から使う。
