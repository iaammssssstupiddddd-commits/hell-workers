...
- **容量制限 (10個)**: 1つの備蓄場所の最大容量は **10個** です。
    - **同一フレーム競合の回避**: 複数のワーカーが同時に到着した際でも、フレーム内の重複ドロップを追跡することで、上限を超えないよう制御されています。
...
- **自動指定**: `bucket_auto_haul_system` により、ドロップされたバケツは `BelongsTo` で紐付いたタンクのストレージへ自動的に搬送 (`Haul`) 指示が付与されます。
...
- **Sand/Rock**:
    - 周辺のアイテムから検索対象を絞り込む。
    - `HaulReservationCache` を使用して、「搬送中 + 予約済み」の数が容量を超えないように厳密に管理する。
- **Water**:
