...
- 同一グリッド・同一資源種別のアイテムは論理 entity と可視状態を維持したまま、最小 Entity ID の1件だけを代表スプライトとして描画します。これにより採掘ドロップや Stockpile の全スプライトが重なって暗い矩形になることを防ぎます。
- 代表選択とスプライト alpha の同期は毎フレーム行うため、代表アイテムが搬出されても残りの1件が直ちに表示を引き継ぎます。
