...
| 関連計画 | [UI操作性改善 実装計画](../plans/ui-usability-improvements-plan-2026-09-14.md)（Draft、実装未着手） |
| 関連Issue/PR | N/A |
...
- 一時停止中に許可するのは閲覧・設計予約までか、配属/方針変更も含むか。
- 即時カメラfocusと単クリック詳細のどちらを既定にするか。実際の管理作業で比較する。
- 日本語を既定UI言語にするか。用語辞書と将来の言語切替を分けて決める。
- 対応する最小画面と文字拡大の範囲。現在のUI scale上限を無条件に引き上げず、到達性を先に整える。

## 12. AI引継ぎメモ

- 現在地: 調査・提案と関連計画の整備まで完了。U01〜U32の実装は未着手。コード編集を他agentへ委譲しない。
- 次の作業: [関連計画](../plans/ui-usability-improvements-plan-2026-09-14.md)のM0でcurrent HEADと再現条件を確認し、M1/M2のP1全8項目から着手する。実行順・依存・進捗は計画を正本とする。
