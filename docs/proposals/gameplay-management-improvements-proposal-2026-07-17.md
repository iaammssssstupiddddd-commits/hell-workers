...
計画状態: `2026-07-20` に B1〜B3 を独立した実装計画へ分割した。B1 と B2 は機能として相互に依存しないが、
両方が Familiar task ranking へ加える方針優先度は、既存 worker score 後の共有 additive offset 契約を使う。
どちらを先に実装しても同じ extension point を作り、後続トラックはそれを再利用して組合せ回帰を固定する。
B3 は Soul Spa 稼働枠と個別給電を段階導入する。共通の container format 変更や C3 を前提にせず、
durable component の追加と runtime state 再構築を各サブトラックで完結させる。

実装状態: B1 は `2026-07-22` に M1〜M5 を完了した。durable policy と旧 save 補完、priority tier 別 request、
manual destination、wheelbarrow grant-time evaluator、通常 Familiar の共有 no-clamp policy score offset に加え、
割当直前 resolver、通常 / wheelbarrow の committed execution、`NewOutbound` / draining 対応 consolidation、
...
- 搬入優先度は既存 i32 task priority へ畳み込まず、現行 worker score 算出後の共有 policy score offset として
  B2 の Familiar 優先度と加算する。各 Normal は 0、最終 score は clamp しない。
...
- WorkType ごとの許可/優先度は、実行時 AI 状態と分離した `FamiliarPolicy` として設計する。
  活動範囲は既存の persistent `TaskArea` を唯一の正本として再利用する。
- WorkType 優先度は既存 i32 task priority へ畳み込まず、現行 worker score 算出後の共有 policy score offset として
  B1 の搬入優先度と加算する。default / Normal の既存 score と Top-K は完全に維持する。
...
- 同順位では安定した tie-break を使い、供給境界付近では復旧 hysteresis margin により
  毎 tick の給電/遮断反転を防ぐ。
- 発電、配線接続、消費、遮断理由を共通の検査レンズで表示する。
- Battery は需要制御と HVAC/Plumbing の消費設備が安定した後の拡張とする。
- 同一グリッドの全消費設備を一律 on/off する現行挙動は、優先度導入後に置き換える。
- user-local setting で priority allocation を無効化した場合は、個別 runtime state を維持したまま現行 all-or-none 配電を再現する。
...
実装状態: `2026-08-08` にM1〜M5を完了し、計画をarchiveした。durable order/save再構築、
Familiar/Soulのexact task終端、Orders single-click/hover、Operation policy、Task Dashboard owner cancel、
Tank/Mixer/Rest/Parkingのatomic recovery、structure / Soul Spa / Room / Power cleanupを実装した。save/load後の
runtime参照reset・stale epoch非変更、Help/恒久docs、Intel Arc/Vulkan/Xlib native V1〜V5、medium 100棟の
fixed-step性能監査20/20、full workspace gateまで完了した。詳細な設計・検証記録は
`docs/plans/archive/building-deconstruction-plan-2026-08-03.md` を参照する。
...
- `UserFacingNotification` と、操作別の typed Outcome Message
...
- [x] C1初版の解体回収は建築別固定tableとし、difficulty/Edictと投入履歴は対象外にする。
  ...
  `docs/plans/archive/building-deconstruction-plan-2026-08-03.md`を正本とする。
...
| `2026-07-20` | `Codex` | Track B を B1 Stockpile、B2 Familiar、B3 Soul Energy の3計画へ分割。初版 policy、既存 TaskArea 再利用、no-kick / priority load shedding、durable component 追加の v1 migration を確定 |
| `2026-07-21` | `Codex` | B1/B2 shared policy score offset、B3 の無効化可能境界、legacy runtime-derived state stripping の v1 例外条件を計画レビューから反映 |
| `2026-07-22` | `Codex` | B1 M1〜M2 の durable policy、旧 save 補完、tier 別 request、manual / wheelbarrow evaluator、通常 Familiar shared score offset を実装 |
| `2026-07-22` | `Codex` | B1 M3 の割当直前 NewInbound 再検証、committed 通常 / wheelbarrow 荷下ろし、NewOutbound / draining 対応 consolidation を実装 |
| `2026-07-22` | `Codex` | B1 M4 の managed Stockpile選択・inspection/editor、単一/矩形共通typed request、部分適用outcome、ToastOnly通知、reset回帰を実装 |
