...
- 同じキーが複数の操作に割り当てられている。F5 はセーブと Soul mask 切替、
  F9 はロードと追加ライト切替、数字キーは時間速度と Familiar 指令の両方に使われる。
  また、TaskArea の `Ctrl+V` と elevation 表示の `V` のように、修飾キーを含む操作が
  別系統へ伝播し得る組み合わせもある。
...
- 進捗: `提案初版 100% / 採否 0% / 実装 0%`
- 直近で完了したこと: 現行コード、恒久ドキュメント、既存提案を横断し、
  重複を避けた優先順位と依存関係を本提案へ整理した。
- 現在のブランチ/前提: `master`。本提案作成時点では Rust 実装を変更していない。

### 次のAIが最初にやること

1. ユーザーと Track A〜D の採否、および最初に着手するサブトラックを確定する。
2. 採用サブトラックについて、該当コードと一次 API 情報を再確認する。
3. `docs/plans/plan-template.md` に従い、1 サブトラックだけを対象とした実装計画を作る。
...
- [ ] 実装へ進むサブトラックの `docs/plans/...` が作成されている
