...
- **feature map**: startup の `build_terrain_feature_map` が `GeneratedWorldLayout.masks` から `Rgba8Unorm` の `TerrainFeatureMap` を生成する。R=`shore sand`、G=`inland sand`、B=`rock field`、A=`zone bias`（grass zone / neutral / dirt zone）。worldgen snapshot を初期値として焼き、runtime では自然障害物の最後の除去時に該当セルの B を 0 へ更新して、採掘済みセルから岩場補正を外す。
...
- **RtT の論理 viewport 契約**: `RttRuntime.viewport` は Window の物理解像度に RtT quality（High=1.0 / Medium=0.75 / Low=0.5）を掛けた texture size を持つ。単一の RtT Camera3d は `ImageRenderTarget.scale_factor = Window scale factor × RtT quality` を明示し、texture size の整数丸め誤差内で `viewport / scale_factor` が Window の論理サイズと一致するようにする。resize・DPI・quality のいずれかが変わった場合はScene textureとtargetを再生成・再bindし、2D前景との投影倍率を維持する。
- **障害物除去後の更新**: `hw_world::obstacle_sync_system` が source-aware な差分同期を行い、自然物由来 blocker の最後の削除でのみ `TerrainChangedEvent`（`Message`）を発行する。岩場は論理 terrain が初めから Dirt でも描画 feature の解除が必要なため、terrain type の値が変わらない場合も発行する。`bevy_app::terrain_metadata_sync_system` は受信して `TerrainIdMap` の該当ピクセルと `TerrainFeatureMap` の rock field channel を同期する。**chunk entity の再生成は不要**（shader が world-space で texture を参照するため、texture 1 ピクセル更新だけで全 chunk の見た目が更新される）。
