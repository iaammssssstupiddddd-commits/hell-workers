...
- **RtT の論理 viewport 契約**: `RttRuntime.viewport` は Window の物理解像度に RtT quality（High=1.0 / Medium=0.75 / Low=0.5）を掛けた texture size を持つ。2 台の RtT Camera3d は `ImageRenderTarget.scale_factor = Window scale factor × RtT quality` を明示し、texture size の整数丸め誤差内で `viewport / scale_factor` が Window の論理サイズと一致するようにする。resize・DPI・quality のいずれかが変わった場合は scene / Soul mask texture と target を同時に再生成・再 bind し、2D 前景との投影倍率を維持する。
