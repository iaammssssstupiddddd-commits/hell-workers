...
- Codex（`.codex/skills/`）や Claude Code プラグイン（`.claude-plugin/skills/`）と併用する場合は、手順の正本を `.cursor/skills/` に揃えると重複が減る。
...
| ツール | 用途 | インストール |
|:---|:---|:---|
| **bacon** | ファイル変更監視 + 自動 `cargo check` | `CARGO_HOME=/home/satotakumi/.cargo cargo install bacon` |
| **cargo-expand** | Bevy derive マクロの展開確認 | `CARGO_HOME=/home/satotakumi/.cargo cargo install cargo-expand` |
| **cargo-udeps** | 未使用依存クレートの検出 | `CARGO_HOME=/home/satotakumi/.cargo cargo install cargo-udeps` |
| **cargo-flamegraph** | フレームグラフによるプロファイリング | `CARGO_HOME=/home/satotakumi/.cargo cargo install flamegraph` |
| **cargo-deny** | 依存ライセンス・脆弱性チェック | `CARGO_HOME=/home/satotakumi/.cargo cargo install cargo-deny` |
...
### マクロ展開確認
```bash
CARGO_HOME=/home/satotakumi/.cargo cargo expand --package hw_core
```
