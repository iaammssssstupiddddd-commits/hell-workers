...
- Familiar AI の `state_decision` / `task_delegation` は root shell として concrete `SpatialGrid` / `WorldMapRead` / `MessageWriter` / `Time` / pathfinding context を保持し、`hw_familiar_ai` 側の状態機械や pure planning layer に narrowed read-only query view や pure context/outcome を橋渡しする。`encouragement` / `auto_gather_for_blueprint` はシステム本体も `hw_familiar_ai` に移設済みであり、bevy_app 側は thin re-export shell のみ。
...
- `apply_task_assignment_requests_system` の登録責務は `hw_soul_ai::SoulAiCorePlugin` が持つ。`task_execution_system` も `hw_soul_ai` に移設済みであり、root 側の `SoulAiPlugin` は再登録せず ordering 参照にのみ使う。`drifting_behavior_system` も同様。
