...
地形は `hw_visual::TerrainSurfaceMaterial` / `TerrainSurfaceMaterialExt` へ移し、`ExtendedMaterial<StandardMaterial, ...>` のまま section clip・lighting・prepass を維持している。startup は `TerrainIdMap`（`R8Unorm`、runtime 更新対象）と `TerrainFeatureMap`（`Rgba8Unorm`、worldgen snapshot）を生成し、`Terrain3dHandles.surface` として共有 `Handle<TerrainSurfaceMaterial>` を全 chunk へ配る。**地形描画は `TerrainChunk` entity（16×16 タイル/chunk、7×7=49 entity）で行い、per-tile の render entity は廃止**。`Tile` marker entity（10,000 個）は描画コンポーネントなしで `WorldMap.tile_entities` に登録され、Familiar AI の収集可否判定の論理 anchor としてのみ機能する。`assets/shaders/terrain_surface_material.wgsl` は `terrain_id_map` を `textureLoad` で引いて center / cardinal 近傍の terrain id を取り、4 アルベド、`terrain_macro_noise`、terrain 種別ごとの `macro_overlay`、`terrain_blend_mask_soft`、`river_flow_noise`、`river_normal_like`、`shoreline_detail`、`terrain_feature_lut` を合成して最終アルベドを決める。shader は world-space UV で参照するため chunk 境界に継ぎ目は生じない。境界ブレンド帯はセル端の狭い範囲に限定し、river を含むブレンドは `river↔sand` の組み合わせだけを通す。`TerrainChangedEvent` の consumer は `terrain_id_map_sync_system` で、`Assets<Image>` 上の id map ピクセルを書き換えるだけで見た目を更新する（chunk entity の再生成は不要）。M1 では `TerrainLodMetrics` / `TerrainLodState` を追加し、`update_terrain_lod_metrics_system` が `Camera3dRtt` の viewport から `tile_rtt_px` を算出する。LOD 判定は `tile_rtt_px` を正本に行い、`tile_screen_px` は `composite_logical_size(window)` 由来の補助観測値としてのみ扱う。詳細は `docs/world_layout.md` の地形レンダリング節。
...
- `update_terrain_lod_metrics_system` は `sync_camera3d_system.after(...)` で登録され、更新済みの `Camera3dRtt` 投影から `tile_rtt_px` / `tile_screen_px` を観測する。`Lod2` は TopDown 専用であり、矢視へ切り替わったフレームで `resolve_lod_level` が必ず `Lod1` へ戻す。
