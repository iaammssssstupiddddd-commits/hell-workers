...
- Familiar AI の `state_decision` / `task_delegation` は root shell として concrete `SpatialGrid` / `WorldMapRead` / `MessageWriter` を保持し、`hw_ai` 側の状態機械には narrowed read-only query view と pure outcome を橋渡しする。
