...
`PreUpdate` は `InputFocusSystems::Dispatch / UiSystems::Focus → CaptureRequest → Resolve →
CaptureTransition → Rollback → CameraGuard → PickingSystems::Hover` の順である。UI button または
keyboard action が overlay open を受理した frame から pending capture を立て、visible capture root への
handoff 中も false→true latch を再発火させない。`Update::GameSystemSet::Input` 内は
`InputResolutionSet::PointerIngress → Consume` の順である。
...
- Group A: command 系 (`assign_task_system`, `familiar_command_input_system`, `stockpile_policy_range_selection_system`, `task_area_selection_system`, `zone_placement_system`, `zone_removal_system`, `task_area_edit_history_shortcuts_system`) を `.chain()` で直列実行する。`TaskContext` / `StockpilePolicyRangeEditState` / `AreaEdit*` / `WorldMapWrite` を共有するため、この並びが唯一の ordering 契約である。
...
- Soulの実行用queryは`TaskQueries`、解除専用queryは`TaskUnassignQueries`が所有する。後者は`ReservationAccess`と`DesignationAccess`だけを集約し、両queryが`TaskReservationAccess`を実装する。割当適用systemは固有query、Familiarの割当は`FamiliarTaskAssignmentQueries`を使う。
...
- Blueprint完成はrootの`commit_completed_building`がliveな完成条件・取消marker・Transform・全footprintのownerを再検証し、WorldMap占有の移譲、完成Entity、companion昇格、Blueprint除去をexclusive commitで確定する。不一致なら適用しない。通行可能建物の予約解除でも別sourceのblockerを保持する。確定後の`publish_building_completed`がEventとpresentation用Messageを各1回配送し、`hw_soul_ai::soul_ai::building_completed::on_building_completed`はWorldMapを読み、footprint marker生成とSoul退避を担当する。`BuildingCompletedVisualMessage`の元`blueprint_entity`は任意操作ガイドの完成照合に使い、Messageはroot inventoryで登録・world置換時clearする。
- `transport_request::producer` の floor/wall 搬入同期は `producer/mod.rs` の共通ヘルパー（`sync_construction_requests`, `sync_construction_delivery`）を利用して重複実装を避ける。全プロデューサーのオーナー解決は `AreaBounds`（`hw_core::area` の共通矩形型）に統一し、`collect_all_area_owners` / `find_owner_for_position` で Familiar TaskArea と Yard 境界を同列に処理する。
...
- `crates/hw_world/src/world_replace.rs` がroom domainのworld replacement ownerである。runtime `Room` / `RoomOverlayTile`をdespawnし、lookup/detection/validation Resourceをidempotentにresetする。root `LogicPlugin`はこのleaf callbackを`LoadResetRegistry`へ1回だけ登録する。
...
`SpatialGridOps` trait契約の正本は`hw_world/src/spatial.rs`である。`crates/hw_spatial`はこれをre-exportし、`SpatialIndex<Tag>`に対する唯一のconcrete implと標準Transform updaterを持つ。9個の公開名はcrate所有ZST tagを使うtype aliasである（Soul用の型名は **`SpatialGrid`** — `SoulSpatialGrid`というRust型はない）。custom cell sizeは`SpatialIndex::new(GridData::new(...))`で指定する。内部データの読み取りには`data()`／`position()`／`positions()`を使い、変更はinsert／update／remove／clear／`replace_positions`へ集約する。membershipまたは保存位置が変わったときだけgenerationを進め、同値更新では進めない。`into_data()`はindexの所有権を消費する取出しAPIである。`ResourceSpatialGrid`のVisibility policyと`GatheringSpotSpatialGrid`のcenter / Added-only policyは専用systemのまま保持する。`ResourceItem` / `Stockpile` / `TransportRequest`のcomponent特化wrapperは`hw_logistics`にあり、`plugins/spatial.rs`から登録される。`crates/bevy_app/src/systems/spatial/`は削除済みで、`crates/bevy_app/src/plugins/spatial.rs`が`hw_spatial` / `hw_logistics`から直接importする。
...
| 方式 | 用途 | 定義場所 |
|:--|:--|:--|
| `Message` | Request と presentation 通知。`MessagesPlugin` に登録し、遅延可能な system が `MessageReader` で消費する | 主に `crates/hw_core/src/events.rs`。`TaskAssignmentRequest` は `hw_jobs`、Stockpile方針の`StockpilePolicyChangeRequest` / `Outcome`は `hw_logistics` が所有する（root登録は `crates/bevy_app/src/plugins/messages.rs`） |
...
| `Ctrl+C/V/Z/Y`, `Ctrl+Shift+Z` | エリア編集操作 | AreaSelection時。Redo は Ctrl+Y / Ctrl+Shift+Z | `systems/command/area_selection/shortcuts.rs` |
| `Ctrl/Alt+Digit1-3` | Area preset 保存/適用 | AreaSelection時 | `systems/command/area_selection/shortcuts.rs` |
| `Tab/Shift+Tab` | Entity Listフォーカス移動 | text input / ActiveMode 以外。resolver 経由 | `list/interaction/navigation.rs` |
| `P` | DamnedSoul スポーン（カーソル位置） | **Debug 時のみ** | `plugins/interface_debug.rs` |
| `O` | Familiar スポーン（カーソル位置） | **Debug 時のみ** | `plugins/interface_debug.rs` |
