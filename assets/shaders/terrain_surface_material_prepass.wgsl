...
fn section_discard(world_position: vec3<f32>) {
    if terrain_surface.cut_active > 0.5 {
        let dist = dot(
            world_position - terrain_surface.cut_position.xyz,
            terrain_surface.cut_normal.xyz
        );
        if dist < 0.0 || dist > terrain_surface.thickness {
            discard;
        }
    }
}
